...
public class TranscriptomicsView implements SelectionListener {
	...
	public TranscriptomicsView() {
		...
		imageTiling = ResourceManager.getPluginImage("bacnet", "icons/tiling.bmp");
	}
	...
	public void createPartControl(Composite parent) {
		...
		compositeTable.setLayout(new GridLayout(1, false));
		{
			...
			btnExpressionAtlas.setImage(ResourceManager.getPluginImage("bacnet", "icons/compareexpression.bmp"));
	...
	private void setData() {
		...
		bioCondsToDisplay = TabDelimitedTableReader.readList(Database.getInstance().getTranscriptomesArrayPath(), true,
				true);
	...
	public void widgetSelected(SelectionEvent e) {
		if (e.getSource() == btnGenomeViewer) {
			...
			try {
				IRunnableWithProgress thread = new OpenExpressionMatrixAndComparisons(selectedTranscriptomes,
						genomeToComparisons);
