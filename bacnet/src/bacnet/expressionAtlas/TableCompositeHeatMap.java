...
public class TableCompositeHeatMap extends Composite implements SelectionListener {
	...
	public TableCompositeHeatMap(Composite parent, int style, String genomeName, boolean packColumns,
		...
		btnColormapper.setImage(ResourceManager.getPluginImage("bacnet", "icons/colorMapper.bmp"));
		...
		btnSaveText.setImage(ResourceManager.getPluginImage("bacnet", "icons/fileIO/txt.bmp"));
	...
	/**
	 * For comparisons delete the second part of the header and keep only the
	 * BioCondition name
	 */
		...
		for (int i = 0; i < matrixDisplayed.getHeaders().size(); i++) {
			...
			col2.setLabelProvider(new ColumnLabelProvider() {
				...
				public Color getBackground(Object element) {
					...
					if (!row[k].equals("")) {
						double value = Double.parseDouble(row[k]);
						ColorMapper colorMapper = colorMapperList
								.getCorrespondingMapper(matrixDisplayed.getHeader(k - 1));
