 ...
 * Contains persistent variable, used through all the website. It is loaded for
 * each User, it is not shared by users
 * 
 * @author UIBC
 *
 */
...
public class Database {
	 ...
	 * Matrices containing all logFC values in a matrix GenomeElement vs
	 * BioCondition
	 */
	...
	/**
	 * Print out JVM properties And set this.path depending on the Operating System
	 * HAS TO BE RUN AT STARTING OF EVERY PROJECT
	 */
		...
		if (logFCTable.containsKey(genomeName)) {
			...
			if (Database.getInstance().getProjectName() == Database.UIBCLISTERIOMICS_PROJECT
				...
				ExpressionMatrix matrix = ExpressionMatrix
						.load(getLOGFC_MATRIX_TRANSCRIPTOMES_PATH() + "_" + genomeName + "_PRIVATE");
				logFCTable.put(genomeName, matrix);
				return matrix;
			} else {
				System.out.println(getLOGFC_MATRIX_TRANSCRIPTOMES_PATH() + "_" + genomeName);
				ExpressionMatrix matrix = ExpressionMatrix
						.load(getLOGFC_MATRIX_TRANSCRIPTOMES_PATH() + "_" + genomeName);
				logFCTable.put(genomeName, matrix);
				return matrix;
			}
		}
	...
	public ExpressionMatrix getExprProteomesTable(String genomeName) {
		if (exprProteomesTable.containsKey(genomeName)) {
			...
			if (Database.getInstance().getProjectName() == Database.UIBCLISTERIOMICS_PROJECT
				...
				ExpressionMatrix matrix = ExpressionMatrix
						.load(getEXPRESSION_MATRIX_PROTEOMES_PATH() + "_" + genomeName + "_PRIVATE");
				exprProteomesTable.put(genomeName, matrix);
				return matrix;
			} else {
				System.out.println(getEXPRESSION_MATRIX_PROTEOMES_PATH() + "_" + genomeName);
				ExpressionMatrix matrix = ExpressionMatrix
						.load(getEXPRESSION_MATRIX_PROTEOMES_PATH() + "_" + genomeName);
				exprProteomesTable.put(genomeName, matrix);
				return matrix;
			}
		}
		...
		if (probesTiling.size() == 0) {
			try {
				...
				DataInputStream in = new DataInputStream(
						new BufferedInputStream(new FileInputStream(Tiling.PROBES_PATH)));
				int i = 0;
				while (true) {
