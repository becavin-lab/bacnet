...
public class NTermData extends OmicsData implements Serializable {
	...
	public static final String[] TYPE_OVERLAPS = { "annotated TIS", "multiple TIS", "updated TIS", "internal TIS",
			"new TIS", "uncategorized" };
	...
	/**
	 * Link NTerm position (coded as a <code>String</code> to
	 * <code>ArrayList(NTerm)</code><br>
	 * This correspond to the list of mapped sequence which are equivalent to other
	 * NTerm but with different modification type
	 */
	...
	/**
	 * Link <code>Nterm</code> to a list of all <code>Nterm</code> which might be
	 * related to it (e.g.: almost same sequence but proteolyse cut it)
	 */
	 ...
	 * Fill:<br>
	 * <li>HashMap<String, ArrayList<NTerm>> peptides is to make a simple HashMap
	 * between NTerm sequence and a list of corresponding <code>NTerm</code>
	 * <li>HashMap<String, ArrayList<NTerm>> modifications links Nterm position to a
	 * list of corresponding <code>NTerm</code>
	 * 
	 */
	 ...
	 * Fill: HashMap<String, ArrayList<NTerm>>() aTISMap, HashMap<String,
	 * ArrayList<NTerm>>() genes, HashMap<String, ArrayList<TypeModif>>()
	 * geneModification
	 */
	...
	/**
	 * Regroup peptides which overlap -> the assumption is that they come from the
	 * same protein but were created from proteolyse
	 */
	...
	/**
	 * Go through <code>proteolyses</code> map, and find the list of NTerm which
	 * might be associated to an input <code>NTerm</code><br>
	...
	/**
	 * Return a HashMap of <code>NTerm</code> with
	 * <code>nTerm.getTypeOverlap() == typeOverlap</code>
	 * 
	 * @return
	 */
	...
	public NTerm getPreviousNterm(NTerm nTerm, String typeOverlap) {
		...
		for (int i = 1; i < annot.length; i++) {
			if (annot[i][0].equals(nTerm.getName())) {
				...
				/*
				 * If no Nterm has been found we go in the opposite direction and the search the
				 * NEXT NTerm
				 */
				k = i;
	...
	/**
	 * 
	 * Load in: TranscriptomeData.PATH_STREAMING + nTermExpName +
	 * TranscriptomeData.EXTENSION
	 * 
	...
	/**
	 * 
	 * Save in: TranscriptomeData.PATH_STREAMING + this.getName() +
	 * TranscriptomeData.EXTENSION
	 * 
