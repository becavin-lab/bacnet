...
/**
 * 
 * This is the principal data model used in the software for managing big table
 * of expression
 * 
 * 
 * 
...
public class ExpressionMatrix extends OmicsData implements Cloneable, Serializable {
	...
	/**
	 * the rowNames are in a TreeMap to have a matching between names and index in
	 * values array, we navigate easily in the TreeMap which is sorted (using a
	 * modifyed Comparator)
	 */
	private TreeMap<String, Integer> rowNames = new TreeMap<String, Integer>();
	/**
	 * In some cases (clustering for example) we need the row to be specifically
	 * organized We use a List to force the organization of the row A boolean
	 * indicate if this matrix as to be used.
	 */
	private ArrayList<String> orderedRowNames = new ArrayList<String>();
	/**
	 * Indicates wether or not rows need to be ordered thanks to ArrayList<String>
	 * orderedRowNames
	 */
	private boolean ordered = false;
	/**
	 * Column names are in an ArrayList<String> so we can navigate easily, and have
	 * access to the index of the column just by giving an header
	 */
	...
	/**
	 * we put all important informations in a table (which should be of the size of
	 * "values")
	 */
	...
	/**
	 * Create new double[][] values with nbRow and nbColumn All value will then be
	 * equal to 0
	 * 
	 * @param nbRow
	 * @param nbColumn
	...
	/**
	 * Return a list of the data contain in this matrix which correspond to a given
	 * strand
	 * 
	 ...
	 * 
	 * @param includeRowNames is an ArrayList<String> so it is first converted into
	 *                        a TreeSet to avoid duplicates and then the row
	 *                        extraction algorithm is ran
	 * @return
	 */
			...
			for (String rowName : includeRowNames) {
				if (this.getRowNames().containsKey(rowName)) {
					for (int i = 0; i < headerAnnotation.size(); i++) {
						annotationNew[subExpr.getRowNames().get(rowName)][i] = this.getValueAnnotation(rowName,
								headerAnnotation.get(i));
					}
				}
			}
	 ...
	 * 
	 * @param proportion indicate of much of the original rows will be include in
	...
	public static ExpressionMatrix arrayToExpressionMatrix(String[][] array, boolean ordered) {
		...
		for (int j = 1; j < array[0].length; j++) {
			for (int i = 1; i < Math.min(10, array.length); i++) {
				try {
					/*
					 * We search the all column for non void Cell, then we check if it's a Double or
					 * a String
					 */
					// int i = 1;
	...
	/**
	 * Export in an array a List of ExpressionMatrix All ExpressionMatrix from the
	 * list are supposed to have the same headers, and thus the same number of
	 * column
	 * 
	 * The first column = the annotation of each ExpressionMatrix The second column
	 * = the RowNames of each ExpressionMatrix
	 * 
	 * @param firstRowName
	 * @param secondRowName
		...
		for (ExpressionMatrix expr : listExpression) {
			for (String key : expr.getRowNames().keySet()) {
				...
				for (int j = 0; j < expr.getHeaderAnnotation().size(); j++) {
					array[i][j + 2 + expr.getHeaders().size()] = expr.getValueAnnotation(key,
							expr.getHeaderAnnotation().get(j));
				}
				i++;
			}
		}
	...
	/**
	 * Count the total Number of row if we concatenate all ExpressionMatrix in the
	 * ArrayList
	 * 
	 * @param listExpression
	 * @return
	...
	/**
	 * I create this comparator in the case where the key are Integer, we use then
	 * the Integer comparator instead of String --- Because with String normal
	 * comparator 1,2,3,10,12 will be classified as 1,10,12,2,3 ---
	 * 
	 * @author Chris
	 *
	 */
