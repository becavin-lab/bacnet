...
public class Annotation implements Serializable {
	 ...
	 */
	public static String EGDE_SUPPTABLE = GenomeNCBI.PATH_ANNOTATION + "/EGDeSuppAnnot.txt";
	/**
	 ...
	 */
	public static String EGDE_ANNOTATION = GenomeNCBI.PATH_ANNOTATION + "/EGDeAnnotation.txt";
	public static String MOUSE_ANNOTTABLE = Database.getInstance().getPath()
			+ "Analysis/Egd-e Annotation/Annot-Affy-Mouse-MoGene1st.txt";
	...
	public ArrayList<Sequence> getElements(Chromosome chromosome, int beginDraw, int endDraw) {
		...
		for (int i = 1; i < annotation.length && !stop; i++) {
			 ...
			 * draw it Meaning either : beginDraw < begin < endDraw or : beginDraw < end <
			 * endDraw or : begin < beginDraw < endDraw < end
			 */
		...
		/*
		 * Reorganize list to be sure to display first the ncRNA and the genes and
		 * finally sRNAs
		 */
	...
	public String getElementATbp(Chromosome chromosome, int bpPosition) {
		/*
		 * By supposing an equal repartition of every genome element we try to find
		 * quickly a close element near to the bpPosition
		 */
		...
		/*
		 * Now from the genome element at index (approxIndex-2), search the closest
		 * genome element which is in the bpPosition
		 */
		if (approxIndex > 2)
	...
	/**
	 * Return a matrix containing different information on each element of the
	 * genome first chromosome
	 * 
	 * @param genome
	 * @param elements
	 * @return
	...
	public static ExpressionMatrix getAnnotationMatrix(Genome genome, ArrayList<String> elements) {
		...
		for (Chromosome chromo : genome.getChromosomes().values()) {
			ExpressionMatrix matrix = ExpressionMatrix.arrayToExpressionMatrix(chromo.getAnnotation().getAnnotation(),
					true);
			matrices.add(matrix);
		}
	...
	public static String[][] getAnnotationGenes(Genome genome, ArrayList<String> elements) {
		...
		for (int i = 0; i < elements.size(); i++) {
			...
			annotation[index][previousSize + 9] = gene.getProduct().replace('\n', ' ').replace('\t', ' ')
					.replace(';', ' ').replaceAll("	", "");
			annotation[index][previousSize + 10] = gene.getComment().replace('\n', ' ').replace('\t', ' ')
					.replace(';', ' ').replaceAll("	", "");
			annotation[index][previousSize + 11] = gene.getRASTinfo().replace('\n', ' ').replace('\t', ' ')
					.replace(';', ' ').replaceAll("	", "");
