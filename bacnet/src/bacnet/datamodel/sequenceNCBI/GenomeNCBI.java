 ...
 * Data for this genome come from: <it> NCBI.fna (for the sequence) <it>
 * NCBI.gff (for Gene, CDS, and ncRNA lists) If it exists some information are
 * added: <it> Rfam.gff (other info) <it> supp info in a .txt file <it> supp
 * info about RAST results in a .txt file For EGD-e only: <it> Srna.xml are read
...
public class GenomeNCBI {
	...
	/**
	 * Path for all raw data for Genomes : Database.getInstance().getPath() +
	 * "/GenomeNCBI/";
	 */
	public static String PATH_RAW = Database.getInstance().getPath() + "/GenomeNCBI/";
	/**
	 * Path for all Genomes : Database.getInstance().getPath() +
	 * "/GenomeNCBI/Genomes/";
	 */
	public static String PATH_GENOMES = Database.getInstance().getPath() + "/GenomeNCBI/Genomes/";
	/**
	 * Path for all new Genomes : Database.getInstance().getPath() +
	 * "/GenomeNCBI/GenomesNew/"; This might be used if all IDs have been changed on
	 * NCBI servers like it was the cas in Sept 2016
	 */
	...
	/**
	 * Path for all supplementary information for genome annotation :
	 * Database.getInstance().getPath() + "/GenomeNCBI/Annotation/";
	 */
	...
	/**
	 * Create a genome with all chromosomes found If annotation = TRUE , we had the
	 * annotation we can found in other fasta files annotation = FALSE, we read only
	 * the complete genome + eventual plasmids
	 * 
	 * @param genomeName
	 * @param annotation
	 * @throws Exception
	...
	public GenomeNCBI(String genomePath, boolean annotation) throws Exception {
		 ...
		 * and classify the other as plasmid by ordering the list
		 * 
		 */
		...
		for (int i = 0; i < files.length; i++) {
			...
			for (String key : genomeSequences.keySet()) {
				...
				ChromosomeBacteriaSequence chromosome = new ChromosomeBacteriaSequence(
						dnaSequence.getProxySequenceReader());
				/*
		...
		/*
		 * If available we add other informations from external files Mainly
		 * designed/used for Listeria monocytogenes EGD-e
		 */
		if (annotation) {
			// update common info
	...
	/**
	 * Go through codingSequence and nonCodingSequence searching for corresponding
	 * accession or locus
	 * 
	 * @param name
	 * @return
