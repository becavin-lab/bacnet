...
/**
 * Transform excel table describing all N-Terminal peptides found in
 * Listeria<br>
...
public class NTermCreateData {
	 ...
	 */
	public static String[] RAWARRAY_COLUMN = { "Peptide", "position", "modified_sequence", "# spectra", "max score",
			"max trh", "soluble", "insoluble" };
	/**
	 * List of column names when PATHARRYA contains multiple experiments
	 */
	public static String[] RAWARRAY_MULTICOLUMN = { "Peptide", "position", "modified_sequence", "# spectra",
			"max score", "max trh", "soluble", "insoluble", "# spectra trypsin soluble 1",
			"# spectra trypsin insoluble 1", "# spectra trypsin soluble 2", "# spectra trypsin insoluble 2",
	 ...
	 * <code>initializeGenomeSequence()</code> to have the genome on the six frames
	 */
...
//                nTerm.setTssUpCoverage((int) Math.rint(Math.pow(2, tssNGSPlus.getValues()[nTerm.getBegin() - k])));
//                /*
...
//                }
//                nTerm.setTssUpDistance(k);
//                nTerm.setTssUpCoverage((int) Math.rint(Math.pow(2, tssNGSMinus.getValues()[nTerm.getEnd() + k])));
//                /*
//                 * Search TSS downstream
//                 */
//                k = 0;
...
//                if ((nTerm.getEnd() - k) >= 0) {
//                    nTerm.setTssDownDistance(k);
//                    nTerm.setTssDownCoverage((int) Math.rint(Math.pow(2, tssNGSMinus.getValues()[nTerm.getEnd() - k])));
//                }
//            }
	...
	/**
	 * For each <code>NTerm</code> calculates anti Shine-Dalgarno sequence affinity
	 * in the region -20bp before peptide on the genome
	 * 
	 * @param nameNTermData
	 */
	public static void findSDSequence(String nameNTermData) {
		...
		int i = 0;
		for (NTerm nTerm : nTermExp.getElements().values()) {
			double energy = UNAfold.hybridRNA(nTerm.getSDSequence(), nTerm.getName(), Sequence.ANTI_SD_SEQ, "anti-SD",
					false);
	 ...
	 * For each NTerm search if the same peptide and the same modif has been found
	 * in MaxQuant analysis.
	 * 
	 * @param fileName
	 */
	...
	private void mapPeptide() {
		...
		for (int i = 1; i < nTermArray.length; i++) {
			...
			/*
			 * We map all the sequences We search for the PERFECT match using
			 * FileUtils.searchPosition(sequence, genomeAA);
			 */
			...
			/*
			 * There is an ambiguity when the peptide start with a M It can be a L->M, I->M
			 * or V->M We remove this ambiguity by mapping peptides without the M and
			 * searching for not AUG start codon just before
			 */
			if (sequence.charAt(0) == 'M') {
				...
				for (Sequence seq : sequencesTemp) {
					/*
					 * Get map sequence + the first codon (depending of the strand we do -3bp or
					 * +3bp)
					 */
				...
				if (foundLV) {
					if (sequencesSize == 0) {
						/*
						 * The sequence was not mapped before as sequencesSize==0 But its mapped now as
						 * foundLV = true
						 * 
						 * So the sequence is now mapped with a non AUG start codon It's counted as: Non
						 * AUG start codon
						 */
						countLV++;
					} else {
						/*
						 * The sequence map with a M and with a non AUG start codon It's counted as:
						 * Ambiguous
						 */
						countAmbiguous++;
					}
	 ...
	 * From initializeGenomeSequence() we get all 6 possible amino acids sequence of
	 * the genome, and map the peptide<br>
	 * The different mapping coordinates are transformed into nucleotide coordinates
	 * (= genome coordinates)
	 * 
	 ...
	 * For each peptide we extract position found, get the AA sequence from it, and
	 * check if it is the same than peptide sequence
	 */
	private void curateTable() {
		...
		for (int i = 1; i < nTermArray.length; i++) {
			finalArray[i][ArrayUtils.findColumn(finalArray, "Name")] = nTermArray[i][ArrayUtils.findColumn(nTermArray,
					"Peptide")];
			...
			finalArray[i][ArrayUtils.findColumn(finalArray, "# spectra")] = nTermArray[i][ArrayUtils
					.findColumn(nTermArray, "# spectra")];
			finalArray[i][ArrayUtils.findColumn(finalArray, "max score")] = nTermArray[i][ArrayUtils
					.findColumn(nTermArray, "max score")];
			finalArray[i][ArrayUtils.findColumn(finalArray, "max trh")] = nTermArray[i][ArrayUtils
					.findColumn(nTermArray, "max trh")];
			finalArray[i][ArrayUtils.findColumn(finalArray, "soluble")] = nTermArray[i][ArrayUtils
					.findColumn(nTermArray, "soluble")];
			finalArray[i][ArrayUtils.findColumn(finalArray, "insoluble")] = nTermArray[i][ArrayUtils
					.findColumn(nTermArray, "insoluble")];
			...
			if (ArrayUtils.findColumn(nTermArray, "# spectra trypsin soluble 1") == -1) { // if only one experiment is
				...
				finalArray[i][ArrayUtils.findColumn(finalArray,
						"# spectra trypsin soluble 1")] = nTermArray[i][ArrayUtils.findColumn(nTermArray,
								"# spectra trypsin soluble 1")];
				finalArray[i][ArrayUtils.findColumn(finalArray,
						"# spectra trypsin insoluble 1")] = nTermArray[i][ArrayUtils.findColumn(nTermArray,
								"# spectra trypsin insoluble 1")];
				finalArray[i][ArrayUtils.findColumn(finalArray,
						"# spectra trypsin soluble 2")] = nTermArray[i][ArrayUtils.findColumn(nTermArray,
								"# spectra trypsin soluble 2")];
				finalArray[i][ArrayUtils.findColumn(finalArray,
						"# spectra trypsin insoluble 2")] = nTermArray[i][ArrayUtils.findColumn(nTermArray,
								"# spectra trypsin insoluble 2")];
				finalArray[i][ArrayUtils.findColumn(finalArray, "# spectra gluC soluble 2")] = nTermArray[i][ArrayUtils
						.findColumn(nTermArray, "# spectra gluC soluble 2")];
				finalArray[i][ArrayUtils.findColumn(finalArray,
						"# spectra GluC insoluble 2")] = nTermArray[i][ArrayUtils.findColumn(nTermArray,
								"# spectra GluC insoluble 2")];
			}
	...
	private void createNTermData() {
		...
		for (int i = 1; i < peptideArray.length; i++) {
			...
			spectrum[4] = Integer
					.parseInt(peptideArray[i][ArrayUtils.findColumn(peptideArray, "# spectra gluC soluble 2")]);
			spectrum[5] = Integer
	...
	private void initializeGenomeSequence(Genome genome) {
		...
		String seqMinus = seqCromosome.charAt(seqCromosome.length() - 1)
				+ seqCromosome.substring(0, seqCromosome.length() - 1);
	 ...
	 * <li>one with only multiple experiments (contains many columns containing "#
	 * spectra") <br>
	 * <br>
	 * For The first type:<br>
	 * Check if PATHARRAY table is complete = has 8 columns, first column is the
	 * list of peptide, soluble and insoluble contains "TRUE" and "FALSE" and no
	 * "yes"<br>
