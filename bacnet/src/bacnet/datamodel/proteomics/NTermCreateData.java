...
//            }
    ...
    public static void addMaXQuantInfo(NTermData nTermData, String fileName, boolean multipleFiles) {
        if (nTermData.getName().equals(NTerm.NTERM_PROJECT_BHI)) {
        ...
        } else if (nTermData.getName().equals(NTerm.NTERM_PROJECT_SERUM)) {
            String finalFileName = fileName + "_trypsin_soluble.txt";
            addMaXQuantInfo(nTermData, finalFileName, 0);
            finalFileName = fileName + "_trypsin_insoluble.txt";
            addMaXQuantInfo(nTermData, finalFileName, 1);
            finalFileName = fileName + "_GluC_soluble.txt";
            addMaXQuantInfo(nTermData, finalFileName, 4);
            finalFileName = fileName + "_GluC_insoluble.txt";
            addMaXQuantInfo(nTermData, finalFileName, 5);
        }
    }
    ...
    private void curateTable() {
        ...
        // PATHARRAY_COLUMN = {"Peptide","position","modified_sequence","# spectra","max score","max
        ...
        for (int i = 1; i < peptideArray.length; i++) {
            ...
            for (String position : positions) {
                ...
                if (temp.length > 1) {
                    ...
                    // System.out.println(("Seq: "+nTerm.getSequenceAA()+" "+nTerm.getSequencePeptide()));
                }
                k++;
            }
        }
    ...
    private void initializeGenomeSequence(Genome genome) {
        ...
        // bacnet.core.utils.data.FileUtils.saveText(genomeAA, NTermUtils.getPATH()+"Genome aminoacid
        // mapping/"+"GenomeAA.txt");
        // bacnet.core.utils.data.FileUtils.saveText(genomeAACompl, NTermUtils.getPATH()+"Genome aminoacid
        // mapping/"+"GenomeAACompl.txt");
        // bacnet.core.utils.data.FileUtils.saveText(genomeAAMinus, NTermUtils.getPATH()+"Genome aminoacid
        // mapping/"+"GenomeAAMinus.txt");
        // bacnet.core.utils.data.FileUtils.saveText(genomeAAMinusCompl, NTermUtils.getPATH()+"Genome
        // aminoacid mapping/"+"GenomeAAMinusCompl.txt");
        // bacnet.core.utils.data.FileUtils.saveText(genomeAAPlus, NTermUtils.getPATH()+"Genome aminoacid
        // mapping/"+"GenomeAAPlus.txt");
        // bacnet.core.utils.data.FileUtils.saveText(genomeAAPlusCompl, NTermUtils.getPATH()+"Genome
