...
public class NTermFilter {
    ...
    /**
     * Read a cutoff<br>
     * Interprete each cutoff by returning true of false if the NTerm pass the condition<br>
    ...
    /**
     * Find i an NTerm start with a methyonine (START), or if the amino acid just before the peptide is
     * a Methyonine (START-1), or just after (START+1)
     * 
     * @param nTerm
     ...
     * if it corresponds to a regular start codon "ATG" we need a Formyl modificaition to prove its a
     * real translation initiation<br>
     * If the methyonine corresponds to any other type of start codon, we don't need more to prove it's
     ...
     * <li>region of 25bp at the beginning of EGDe RAST gene, if this overlap is smaller than with EGde
     * take this one, in order to detect bona-fide aTIS
     * <li>region of 25bp at the beginning of Srna or ASrna of EGDe, if this overlap is smaller than
     * with Srna take this one, in order to detect bona-fide aTIS
     ...
     * <br>
     * The 25bp are given by <code>aTISCutoff</code> argument of the <code>NTermFilter</code> object
     *
     * @param nTerm
     * @param genome
     * @param genomeRAST
