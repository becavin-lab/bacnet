...
public class NTermUtils {
    ...
    /**
     * From a list of peptide position retrieve the complete sequence of the ORF (in frame) containing
     * the peptide (amino acid sequence) <br>
     * WARNING: Be careful that your peptide info (begin,end) are correct. Test (begin-end+1)/3 = an
     * integer, meaning that it is actually the sequence of a amino acid sequence.<br>
     * If one or two nucleotides are included where they should not be, the algorithm will not work.
     * 
     * @param fileName
     */
    ...
    /**
     * Giving a position and a strand on the genome we extract the full ORF (in frame) containing the
     * peptide (amino acid sequence) <br>
     * 
     * @param seq
     * @param genome
     * @return
     ...
     * NOT FINISHED <br>
     * <br>
     * Search for the different start codon before a peptide, and the stop codon after this peptide<br>
     * Print the nucleotide sequence plus the codon, and identify Start, STOp and the reference Peptide
     * used
     */
    ...
    public static void findSDSequenceforLMO() {
        ...
        for (Sequence seq : egde.getFirstChromosome().getAllElements().values()) {
            if (seq.getName().contains("rli")) {
                double energy =
                        UNAfold.hybridRNA(seq.getSDSequence(), seq.getName(), Sequence.ANTI_SD_SEQ, "anti-SD", false);
