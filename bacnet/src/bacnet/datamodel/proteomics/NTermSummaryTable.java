 ...
 * Using NTermFilter we have regroup TIS in different overlap groups, which we use now to create the
 * Summary Tables.
 * 
 * 
 * @author UIBC
...
public class NTermSummaryTable {
    ...
    /**
     * For every peptide, corresponding to a Type of overlap, create a summary table. When a peptide has
     * been already put in a summary peptide, its other mapped peptide are put in
     * <code>notInSummary ArrayList</code>
     * 
     * @param typeOverlap
     * @param nameNTermData
     * @param alreadyMapped
    ...
    private static String getSummaryTable(String typeOverlap, NTermData nTermData, String pathResult) {
        ...
        if (nTermData.equals(NTerm.NTERM_PROJECT_SERUM)) {
            String[] headersTemp = {"Peptide sequence", "Duplicates mapping", "MaxQuant", "MaxQuantum", "TIS",
                    "MappingFrame", "Soluble", "Insoluble", "Begin", "End", "Strand",
                    "AcD3 Name;AcD3 spectra;AcD3 score;AcD3 threshold", "Ace Name;Ace spectra;Ace score;Ace threshold",
                    "For Name;For spectra;For score;For threshold", "Experiments Spectrum", "Start codon",
                    "Previous codon", "Next codon", "Start code", "Overlap", "Overlap in frame", "Operon",
                    "Description", "COG", "TSS up type (-position)", "TSS up distance", "TSS up coverage",
                    "TSS down type (-position)", "TSS down distance", "TSS down coverage", "Codon Usage", "SD ?",
                    "SD sequence (20bp upstream)", "Found in BHI"};
            headers = headersTemp;
        }
        String header = "";
        ...
        for (NTerm nTerm : nTermData.getNTerms()) {
            if (nTerm.getTypeOverlap() == typeOverlap) {
                ...
                if (nTermData.getName().equals(NTerm.NTERM_PROJECT_SERUM)) {
                    totalSequenceInSerum++;
                    String foundInBHINTerm = "";
                    NTermData nTermDataBHI = NTermData.load(NTerm.NTERM_PROJECT_BHI);
                    // System.out.println(nTerm.getSequencePeptide());
                    for (NTerm nTermBHI : nTermDataBHI.getNTerms()) {
                        // System.out.println(nTermBHI.getSequencePeptide());
                        if (nTerm.getSequencePeptide().equals(nTermBHI.getSequencePeptide())) {
                            foundInBHINTerm = "TRUE;" + nTermBHI.getName() + ";" + nTermBHI.getOverlap();
                            System.out.println(k + "  " + nTerm.getSequencePeptide());
                            System.out.println(foundInBHINTerm);
                            sequenceInSerumAndBHI++;
                        }
                    }
                    row += foundInBHINTerm + "\t";
                    k++;
