...
public class BioCondition implements Serializable {
    ...
    public boolean containTranscriptomes() {
        ...
        for(TypeData dataType : this.getTypeDataContained()) {
            if(dataType == TypeData.DNASeq) found = true;
            if(dataType == TypeData.ExpressionMatrix) found = true;
            if(dataType == TypeData.GeneExpr) found = true;
            if(dataType == TypeData.RiboSeq) found = true;
            if(dataType == TypeData.RNASeq) found = true;
            if(dataType == TypeData.TermSeq) found = true;
            if(dataType == TypeData.Tiling) found = true;
            if(dataType == TypeData.TSS) found = true;
        }
        return found;
    }
