...
public class SaveFileUtils {
	...
	public static void saveControltoPNG(String divID, String fileName, Shell shell) {
		 ...
		 */
		/*
		 * NOT WORKING - GET DIVID and print only the control String $el = "$el"; String
		 * id = WidgetUtil.getId(control); String divId = "test"; String[] scripts =
		 * {"rap.getObject( '", id, "' ).", $el, ".attr( 'div-id', '", divId + "' );"
		 * ,"console.log(document.getElementById(\""+divId+"\"));","console.log(\"yo\")"
		 * }; StringBuilder script = new StringBuilder(); script.append( "try{" ); for(
		 * String str : scripts ) { script.append( str ); } script.append(
		 * "}catch(e){console.log(\"Cannot find div\")}" );
		 */
...
//         }
	 ...
	 * 
	 * @param fileName to replace in the html file;
	 *                 htmlText.replaceFirst("_FileName", fileNameResource);
