...
public class UNAfold {
     ...
     * @param name
     * @param seq
     * @param fileName prefix of the file (Extension will be add automatically), it will be saved in the
     *        path <code>UnaFold.PATH_DATA</code>
    ...
    public static double foldRNA(String name, String seq, String fileName, boolean removeTemp) {
        ...
        try {
            ...
            plotFolding(fileName + ".ct", 350, "Mini-");
            /*
             * Get energy
             */
        ...
        try {
            ...
            for (int i = 0; i < freeEnergies.length; i++) {
                try {
                    ...
                    freeEnergies[i] = ExpressionMatrix.MISSING_VALUE;
                }
            }
            ...
            for (int i = 0; i < annot.length; i++) {
                for (int j = 0; j < annot[0].length; j++) {
                    try {
                        ...
                        annot[i][j] = ExpressionMatrix.MISSING_VALUE;
                    }
                }
            }
    ...
    public static void plotFolding(String fileName, int width, String prefix) {
        ...
        // \""+FileUtils.removeExtension(fileName)+".ann\" -o \""+output+"\" \""+(PATH_DATA+fileName)+"\"";
        try {
            String out = CMD.runProcess(execProcess, true);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
