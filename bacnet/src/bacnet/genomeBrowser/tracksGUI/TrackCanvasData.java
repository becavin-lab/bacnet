...
public class TrackCanvasData extends Canvas implements MouseMoveListener {
	...
	/**
	 * When mouse is clicked we displayed a line is displayed to show synteny:
	 * mouseYPosition gives the X position
	 */
	private int mouseXPosition = 0;
	/**
	 * When mouse is clicked we displayed a line is displayed to show synteny:
	 * mouseYPosition gives the Y position
	 */
	...
	/**
	 * Width in pixel of one base pair (fundamental unit for the rest of the
	 * display)
	 */
	 ...
	 * <li>Set width and height parameters for data and annotation track Display in
	 * order:<br>
	 * <li>different line and ticks for the base pair position, every display needed
	 * in the background
	...
	/**
	 * From the value of the data at a given genomic position, this function return
	 * the position in the <code>GC</code>
	 * 
	...
	private void displayDataBIOCOND(GC gc) {
		...
		for (String bcName : track.getDatas().getBioConditionHashMaps().keySet()) {
			...
			/*
			 * If only RNASeq data are in displayed we need to change the legend of the data
			 * (log transformed)
			 */
	...
	/**
	 * Display sequence = For each nucleotide, give the translation of the codon in
	 * which it is in the center
	 * 
	 * @param gc
	 */
		...
		for (int i : probes.subMap(beginProbe, endProbe).keySet()) {
			...
			for (Tiling tiling : tilings) {
				if (!track.getDatas().getDataNOTDisplayed().contains(tiling.getName())) {
					...
					/*
					 * A tiling probes is a 25bp probes, but depending on bpSizeH it change its size
					 * Size parameter will be used to decide: - if we draw lines between points - if
					 * we display all the data, or just a percentage of them - what will be the
					 * final size of the drawing
					 */
					...
					/*
					 * Display each probes <br> If size < 0.3, the more we got data the less number
					 * of probes we display That is why, display of probes is dependent of
					 * Math.random()<(size*10)
					 */
	...
	/**
	 * Function to display a list of GeneExpressionStreaming data in a GC (Graphic
	 * context)
	 * 
	 * @param gc
	 * @param seqDatas
		...
		for (ExpressionData seqData : seqDatas) {
			if (!track.getDatas().getDataNOTDisplayed().contains(seqData.getName())) {
				...
				for (int i = track.getDisplayRegion().getX1(); i < track.getDisplayRegion().getX2(); i++) {
					...
					/*
					 * Get a random number to decide if we display the point or not If
					 * Math.random()<(bpSizeH)*8), we will have maximum 9000 points displayed
					 * 
					 */
					boolean drawProbe = false;
					if (bpSizeH > 1) {
	...
	private void displayNTerm(GC gc, ArrayList<NTermData> nTerms, double min, double max, int dataIndex,
		...
		for (NTermData dataNTerm : nTerms) {
			if (!track.getDatas().getDataNOTDisplayed().contains(dataNTerm.getName())) {
				...
				/*
				 * reorder to get the formyl in First Position And the highlighted nTerm in the
				 * last position
				 */
				...
				for (TIS tis : tisList) {
					...
					Rectangle rectangle = new Rectangle(position[0], position[1], (int) geneSize,
							positionZero[1] - position[1]);
					if (!tis.isStrand())
	...
	/**
	 * Function to display a list of ExpressionMatrix data in a GC (Graphic
	 * context)<br>
	 * All rowNames should correspond to a locus on the genome, otherwise it will be
	 * ignore
	 * 
	 * @param gc
	 * @param geneExprs
	...
	/**
	 * Display all decorative elements which will be draw at the end, so on every
	 * other elements
	 * 
	 * @param gc
	 */
	...
	/**
	 * If the mouse is over the genome viewer, detect it is on the annotation, and
	 * display the corresponding gene, sRNA, asRNA
	 * 
	 * @param e
	 */
	@Override
