...
/**
 * CRISPRGuideView for CRISPRGuide datasets
 * 
 * @author christophebecavin
 *
 */
public class CRISPRGuideView implements SelectionListener {
    ...
    public static final String ID = "bacnet.CRISPRGuideView"; //$NON-NLS-1$
    ...
    public void createPartControl(Composite parent) {
        ...
        lblClickOneTime.setText("Click on a CRISPR Guide row to visualize it on the Genome Viewer");
        ...
        btnSaveTxt.setToolTipText("Download CRISPR guide table");

        Label lblSaveSelectionAs = new Label(composite_2, SWT.NONE);
        lblSaveSelectionAs.setText("Download CRISPR guide table");
        lblSaveSelectionAs.setFont(SWTResourceManager.getBodyFont(10, SWT.NORMAL));
        btnSaveTxt.addSelectionListener(this);
        {
            tableBioCondition =
                    new TableViewer(compositeSummary, SWT.BORDER | SWT.FULL_SELECTION | SWT.MULTI | SWT.VIRTUAL);
            ...
            tableBioCondition.addSelectionChangedListener(new ISelectionChangedListener() {
                ...
                public void selectionChanged(SelectionChangedEvent event) {
                    ...
                    int guidePosition = Integer
                            .parseInt(tableBioCondition.getTable().getItem(i).getText(columnNames.indexOf("Start")));
                    GenomeTranscriptomeView.displayCRISPROmics(partService, guidePosition);
                }
            });

        }
        scrolledComposite.setContent(compositeSummary);
        // scrolledComposite.setMinSize(compositeSummary.computeSize(SWT.DEFAULT,
        // SWT.DEFAULT));
        setData();
    }
    ...
    private void setData() {
         ...
         * Load Table
         */
        String cripsguideArrayPath =
                Database.getANNOTATIONDATA_PATH() + Database.getInstance().getDatabaseFeatures().get("GUIDES");
        bioCondsArray = TabDelimitedTableReader.read(cripsguideArrayPath);
        bioConds = TabDelimitedTableReader.readList(cripsguideArrayPath, true, true);
        bioCondsToDisplay = TabDelimitedTableReader.readList(cripsguideArrayPath, true, true);
        bioCondsToDisplay.remove(0);
        bioConds.remove(0);
        updateGenomeTable();
        lblXxSrnas.setText("Browse through " + (bioCondsArray.length - 1) + " CRISPR Guides");
    ...
    public void pushState() {
        ...
        if (this.txtSearch.getText() != "") {
            ...
            NavigationManagement.pushStateView(CRISPRGuideView.ID, parameters);
        } else {
     ...
     * @param partService
     */
    public static void displayCRISPRTable(EPartService partService, HashMap<String, String> parameters) {
        partService.showPart(ID, PartState.ACTIVATE);
        // update data
        MPart part = partService.findPart(ID);
        CRISPRGuideView view = (CRISPRGuideView) part.getObject();
    ...
    public void widgetSelected(SelectionEvent e) {
        if (e.getSource() == btnSaveTxt) {
            ...
            if (bioCondsToDisplay.size() < 100) {
                ...
                SaveFileUtils.saveTextFile("CRISPR_Guide_Table.txt", arrayRep, true, "", arrayRepHTML, partService,
                        shell);
            } else {
                SaveFileUtils.saveTextFile("CRISPR_Guide_Table.txt", arrayRep, true, "", "", partService, shell);
            }
            ...
            if (dialog.open() == 0) {
                ...
                bioCondsToDisplay.clear();
                for (String[] row : bioConds) {
                    String gene = row[columnNames.indexOf("OverlapGene")];
                    if (!gene.equals("")) {
