...
public class DataTrack {
	...
	/**
	 * True if we display Absolute value expression (Log(Value)) False if we display
	 * Relative value expression (Log(FC))
	 */
	private boolean displayAbsoluteValue = true;

	/**
	 * Indicate if at a specific position of the genome : data are displayed, or
	 * just leave in grey color
	 */
	 ...
	 * <br>
	 * OVERLAYBC : Color are always the same, blue and red for tiling, black for
	 * GeneExpr, other color for the rest <br>
	...
	public void addBioCondition(String bioCondName) {
		...
		if (bioCond.getOmicsData().size() != 0 || bioCond.getComparisons().size() != 0) {
			/*
			 * If the biological condition we add has no Absolute value expression data, we
			 * force the view to switch to relative expression mode
			 */
			if (bioCond.getOmicsData().size() == 0 && bioCond.getComparisons().size() != 0) {
			...
			if (displayAbsoluteValue) {
				...
				if (bioCond.getComparisons().size() != 0) {
					for (String bioCond2Name : bioCond.getComparisons()) {
						BioCondition comparisonBioCond = bioCond.compare(BioCondition.getBioCondition(bioCond2Name),
								false);
	...
	/**
	 * Changed all BioCondition to display when switching from "ABSOLUTE VALUE
	 * DISPLAY3 to "RELATIVE VALUE DISPLAY"
	 */
	...
	/**
	 * Changed all BioCondition to display when switching from "RELATIVE VALUE
	 * DISPLAY3 to "ABSOLUTE VALUE DISPLAY"
	 */
	public void relativeTOabsoluteValue() {
		...
		for (BioCondition comparisonBioCond : bioConditionHashMaps.values()) {
			if (comparisonBioCond.getName().contains(" vs ")) {
				BioCondition bioCond1 = BioCondition
						.getBioCondition(BioCondition.parseName(comparisonBioCond.getName())[0]);
				BioCondition bioCond2 = BioCondition
						.getBioCondition(BioCondition.parseName(comparisonBioCond.getName())[1]);
				boolean added = false;
	...
	/**
	 * Add BioCondition in BioConditionHashMaps respecting a certain order: 1 -
	 * RNASeq 2 - Tiling 3 - Gene Expression 4 - ExpressionMatrix 5 - TSS 6 -
	 * Proteome 7 - Nterm 8 - Unknown
	 * 
	 * @param bioCond
	 */
