...
public class GeneViewLocalizationTools {
	...
	public static void loadLocalizationFigure(Browser browserLocalization, String[][] arrayGeneToLocalization,
		...
		String[] geneLocalization = info[ArrayUtils.findColumn(arrayGeneToLocalization, "Subcellular location")]
				.split("/");
		for (String compartment : geneLocalization) {
			if (!compartment.equals("Unknown")) {
				String nameCompartment = SubCellCompartment.COMPARTMENT_NAMES[TypeCompartment.valueOf(compartment)
						.ordinal()];
				/*
				 * Some of the compartment names are displayed in two lines so we need to modify
				 * the 2 lines in the SVG to display them in RED
				 */
