...
public class FastaFileReader<S extends Sequence<?>, C extends Compound> {
	...
	/**
	 * 
	 * From a .ffn file in root/file.faa We had all CodingInfo into the
	 * ChromosomeSequence
	 * 
	...
	public static LinkedHashMap<String, ChromosomeBacteriaSequence> addCodingRegionFeaturesFromFFN(File root, File file,
		...
		for (int i = 0; i < files.length; i++) {
			...
			if (FileUtils.removeExtensionAndPath(files[i].getAbsolutePath())
				...
				FastaFileReader<DNASequence, NucleotideCompound> fastafileReader = new FastaFileReader<DNASequence, NucleotideCompound>(
	...
	/**
	 * 
	 * From a .frn file in root/file.frn We add all Noncoding Info into the
	 * ChromosomeSequence
	 * 
	 * This method use my personal NCBIFastaHeaderParser
	 * 
	...
	/**
	 * According to the info we found in Chromosomes, we update the info of the
	 * different components (coding, ncoding, proteins)
	 * 
	 * @param chromosomes
	 * @return
	 */
	 ...
	 * 
	 * ******* There is a problem when the genes in the fasta file have the same Id,
	 * so I modify this method accordingly
	 * 
	 * @return
	 * @throws Exception
	 */
					...
					if (sb.length() > 0) {
						...
						if (compoundSet instanceof AminoAcidCompoundSet) {
							for (int j = 0; j < sequ.length(); j++) {
								...
								AminoAcidCompound compound = ((AminoAcidCompoundSet) compoundSet)
										.getCompoundForString(nucl);
								if (compound == null) {
