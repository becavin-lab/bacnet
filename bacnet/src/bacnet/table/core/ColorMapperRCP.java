...
public class ColorMapperRCP implements Serializable, Cloneable {
	...
	private transient Type type = Type.OTHER;
	private transient TreeSet<String> columnNames = new TreeSet<String>();
	...
	private transient Color headerColor;
	private transient Color textHeaderColor;
	private transient Color textColor;

	private Font fontString = new Font("Arial", Font.BOLD, 12);
	private Font fontDouble = new Font("Arial", Font.PLAIN, 12);;
	...
	public ColorMapperRCP(Shell shell) {
	...
	public java.awt.Color parseColorAWT(double value) {
		// find between which bound this value is
		Color color = parseColor(value);
		java.awt.Color awtColor = swtColorToAwt(color);
		return awtColor;
	}

	public static java.awt.Color swtColorToAwt(Color color) {
	...
	public void setValues(ExpressionMatrix exprMatrix) {
		...
		setStat();
		if (type.equals(Type.STDEV)) {
			setDefaultPos();
			setStdevDataColor();
		} else if (type.equals(Type.STAT)) {
			min = 0;
			max = 1;
			setStatMapper();
		} else {
	...
	public void setDefaultColor() {
		...
		headerColor = new Color(display, 200, 200, 200);
		textHeaderColor = new Color(display, 0, 0, 0);
	...
	@Override
	public ColorMapperRCP clone() {
		ColorMapperRCP cloned;
		try {
			cloned = (ColorMapperRCP) super.clone();

			// Color does not implement clone so we need to create a copy by hand
			cloned.setCenterColor(cloneColor(this.centerColor));
			cloned.setHeaderColor(cloneColor(this.headerColor));
			...
			cloned.setTextHeaderColor(cloneColor(this.textHeaderColor));
			return cloned;
	...
	public static ColorMapperRCP load(String fileName, Shell shell) {
		if (fileName != null) {
			try {
				...
				ColorMapperRCP colorMapper = (ColorMapperRCP) in.readObject();
				...
				colorMapper.setHeaderColor(colorMapper.getHeaderColorSWT().toColor(display));
				colorMapper.setTextHeaderColor(colorMapper.getTextHeaderColorSWT().toColor(display));
	...
	public void save(String fileName) {
		if (fileName != null) {
			try {
				...
				headerColorSWT = new ColorSWT(headerColor);
				textHeaderColorSWT = new ColorSWT(textHeaderColor);
	...
	public Color getTextHeaderColor() {
		return textHeaderColor;
	}

	public void setTextHeaderColor(Color textHeaderColor) {
	...
	public Font getFontString() {
		return fontString;
	}

	public void setFontString(Font fontString) {
