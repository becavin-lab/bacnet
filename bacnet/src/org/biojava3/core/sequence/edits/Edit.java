...
/**
 * Interface for carrying out edit operations on a Sequence. The 3 major methods of Editing are
 * supported
 *
 * <ul>
 * <li>Insertion</li>
 ...
 * </ul>
 *
 * The interface is provided so end users can use our implementations, which are implementations
 * which attempts to create views of Sequences in an editted form not a full-realised editted
 * Sequence, or their own.
 *
 * @author ayates
...
public interface Edit<C extends Compound> {
    ...
    /**
     * Abstract class which defines all edit operations as a call to discover what 5' and 3' ends of an
     * editing Sequence should be joined together with a target Sequence. These ends can be of 0 length
     * but conceptionally they can still exist.
     */
    public static abstract class AbstractEdit<C extends Compound> implements Edit<C> {

        /**
         * Should return the 5-prime end of the given Sequence according to the edit. An empty Sequence is
         * valid.
         */
        protected abstract Sequence<C> getFivePrime(Sequence<C> editingSequence);

        /**
         * Should return the 3-prime end of the given Sequence according to the edit. An empty Sequence is
         * valid.
         */
         ...
         *
         * @param editingSequence Asked for in-case we need to do String to Sequence conversion so we need a
         *        CompoundSet which is given by the Sequence we are editing
    ...
    /**
     * Edit implementation which allows us to insert a base at any position in a Sequence. Specifying 1
     * base is used to insert at the start and end of a Sequence. If you wish to carry out an
     * in-sequence insertion then you specify the flanking base positions e.g.
     *
     * <pre>
            ...
            if (singlePosition) {
                if (getStart() == 1) {
                    ...
                    throw new IllegalStateException("Given one position to "
                            + "insert at but this is not the start or end " + "of the Sequence; cannot support this");
                }
            }
    ...
    public static class Substitute<C extends Compound> extends AbstractEdit<C> {
        ...
        /**
         * Must use this rather than the no-args getEnd as this can return -1 and the length of a sub is
         * dependent on the length of the Sequence; we cannot assume 1:1 mapping between characters in a
        ...
        protected Sequence<C> getThreePrime(Sequence<C> editingSequence) {
            ...
            if (end > editingSequence.getLength()) {
                throw new IndexOutOfBoundsException(end + " is greater than the max index of "
                        + "the editing sequence (" + editingSequence.getLength());
