...
/**
 * Provides a set of static methods to be used as static imports when needed across multiple
 * Sequence implementations but inheritance gets in the way.
 *
 * It also provides a place to put utility methods whose application can be to a single class of
 * Sequence e.g. {@link NucleotideCompound} {@link Sequence}; or to any Sequence e.g. looking for
 * the {@link #getComposition(Sequence)} or {@link #getDistribution(Sequence)} for any type of
 * Sequence.
 *
 * All of these methods assume that you can use the {@link Iterable} interface offered by the
 * implementations of {@link Sequence} to provide all the compounds that implementation allows you
 * to see. Since sequence should know nothing about its backing stores (apart from calling out to
...
public class SequenceMixin {
    ...
    /**
     * For the given vargs of compounds this method counts the number of times those compounds appear in
     * the given sequence
     *
     ...
     *
     * @param sequence The {@link NucleotideCompound} {@link Sequence} to perform the GC analysis on
     ...
     *
     * @param sequence The {@link NucleotideCompound} {@link Sequence} to perform the AT analysis on
    ...
    /**
     * Analogous to {@link #getComposition(Sequence)} but returns the distribution of that
     * {@link Compound} over the given sequence.
     *
     * @param <C> The type of compound to look for
     * @param sequence The type of sequence to look over
     * @return Returns the decimal fraction of the compounds in the given sequence. Any compound not in
     *         the Map will return a fraction of 0.
     */
    ...
    /**
     * Does a linear scan over the given Sequence and records the number of times each base appears. The
     * returned map will return 0 if a compound is asked for and the Map has no record of it.
     *
     * @param <C> The type of compound to look for
     * @param sequence The type of sequence to look over
    ...
    /**
     * Used as a way of sending a Sequence to a writer without the cost of converting to a full length
     * String and then writing the data out
     *
    ...
    /**
     * For the given Sequence this will return a {@link StringBuilder} object filled with the results of
     * {@link Compound#toString()}. Does not used
     * {@link #write(java.lang.Appendable, org.biojava3.core.sequence.template.Sequence) } because of
     * its {@link IOException} signature.
     */
    ...
    /**
     * Shortcut to {@link #toStringBuilder(org.biojava3.core.sequence.template.Sequence)} which calls
    ...
    /**
     * For the given {@link Sequence} this will return a {@link List} filled with the Compounds of that
     * {@link Sequence}.
     */
    ...
    /**
     * Performs a linear search of the given Sequence for the given compound. Once we find the compound
     ...
     * {@link ReversedSequenceView} and passing it into {@link #indexOf(Sequence, Compound)}. We then
     * inverse the index coming out of it.
     */
    ...
    /**
     * Creates a simple sequence iterator which moves through a sequence going from 1 to the length of
     * the Sequence. Modification of the Sequence is not allowed.
     */
    ...
    /**
     * Implements sequence shuffling by first materializing the given {@link Sequence} into a
     * {@link List}, applying {@link Collections#shuffle(List)} and then returning the shuffled elements
     * in a new instance of {@link SequenceBackingStore} which behaves as a {@link Sequence}.
     */
    ...
    /**
     * A case-insensitive manner of comparing two sequence objects together. We will throw out any
     * compounds which fail to match on their sequence length & compound sets used. The code will also
     * bail out the moment we find something is wrong with a Sequence. Cost to run is linear to the
    ...
    /**
     * A case-sensitive manner of comparing two sequence objects together. We will throw out any
     * compounds which fail to match on their sequence length & compound sets used. The code will also
     * bail out the moment we find something is wrong with a Sequence. Cost to run is linear to the
     * length of the Sequence.
     *
    ...
    /**
     * A basic sequence iterator which iterates over the given Sequence by biological index. This
     * assumes your sequence supports random access and performs well when doing these operations.
     *
     * @author ayates
     *
