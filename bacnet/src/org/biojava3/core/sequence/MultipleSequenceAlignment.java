...
public class MultipleSequenceAlignment<S extends Sequence<C>, C extends Compound> implements LightweightProfile<S, C> {
    ...
    public String toString(StringFormat format) {
        switch (format) {
            ...
            case CLUSTALW:
            default:
                return toString(60, String.format("CLUSTAL W MSA from BioJava%n%n"),
                        IOUtils.getIDFormat(sequences) + "   ", true, false, true, false);
            case FASTA:
            ...
            for (int i = 0; i < getLength(); i += width) {
                ...
                if (aligIndices) {
                    if (end < i + width) {
                        ...
                        aligIndFormat =
                                "%-" + Math.max(1, line / 2) + "d %" + Math.max(1, line - (line / 2) - 1) + "d%n";
                    }
                    if (idFormat != null) {
