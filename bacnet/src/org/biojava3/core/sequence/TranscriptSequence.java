...
public class TranscriptSequence extends DNASequence {
    ...
    /**
     * http://www.sequenceontology.org/gff3.shtml http://biowiki.org/~yam/bioe131/GFF.ppt
     * 
     * @return
     */
    /**
     * Return a list of protein sequences based on each CDS sequence where the phase shift between two
     * CDS sequences is assigned to the CDS sequence that starts the triplet. This can be used to map a
     * CDS/exon region of a protein sequence back to the DNA sequence If you have a protein sequence and
     * a predicted gene you can take the predict CDS protein sequences and align back to the protein
     * sequence. If you have errors in mapping the predicted protein CDS regions to an the known protein
     * sequence then you can identify possible errors in the prediction
     *
     * @return
     */
    public ArrayList<ProteinSequence> getProteinCDSSequences() {
        ...
        for (int i = 0; i < cdsSequenceList.size(); i++) {
            ...
            // System.out.println("CDS " + getStrand() + " " + cdsSequence.getPhase() + "="
            // + codingSequence);
