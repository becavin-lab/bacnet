...
public class ProteinSequence extends AbstractSequence<AminoAcidCompound> {
    ...
    /**
     * A protein sequence where the storage of the sequence is somewhere else with user defined set of
     * amino acids. Could be loaded from a large Fasta file or via a Uniprot Proxy reader via Uniprot ID
     * 
     * @param proxyLoader
     */
    public ProteinSequence(ProxySequenceReader<AminoAcidCompound> proxyLoader,
    ...
    /**
     * A Protein sequence can be stand alone or loaded from a transcript sequence. The design goal is to
     * allow the creation of a Protein sequence from a Uniprot ID or some other Protein ID that based on
     * cross reference you should be able to get the GeneSequence that codes for the protein if the
     * CDS/Gene region is known. From the GeneSequence you should then be able to get the
     * ChromosomeSequence which then allows you explore flaning regions of the gene sequences. The
     * framework is in place to do this but currently hasn't been implement in the reverse direction
     * starting from the Protein sequence.
     *
    ...
    public static void main(String[] args) {
        ...
        StringProxySequenceReader<AminoAcidCompound> sequenceStringProxyLoader =
                new StringProxySequenceReader<AminoAcidCompound>("XRNDCEQGHILKMFPSTWYVBZJA",
