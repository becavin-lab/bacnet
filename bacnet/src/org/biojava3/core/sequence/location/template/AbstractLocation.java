...
/**
 * Base abstraction of a location which encodes for the majority of important features about a
 * location such as the start, end and strand
 *
 * @author ayates
 */
public abstract class AbstractLocation implements Location {
     ...
     * @param circular Boolean which says if the current location was circular or not
     * @param betweenCompounds Indicates the location lies at the position between a pair of bases;
     *        means the bases must be next to each other (and therefore cannot be complex)
    ...
    public AbstractLocation(Point start, Point end, Strand strand, boolean circular, boolean betweenCompounds,
        ...
        /**
         * I just add this in order to fix return iof an exception when the list if null
         * 
         * @Becavin Christophe
         */
    ...
    /**
     * Returns the normalised list of sub locations i.e. only those locations which do not have a sub
     * location. Useful for when you need to get the exact elements of a location back for sub
     * sequences.
     */
    ...
    /**
     * If circular this will return the sequence represented by the sub locations joined. If not
     * circular then we get the Sequence for the outer-bounds defined by this location.
     */
    ...
    /**
     * Reverses and (if possible) complements the Sequence so as to represent the reverse strand (if one
     * exists). Also does checking to see if the location we are on is on the reverse strand or not.
     */
