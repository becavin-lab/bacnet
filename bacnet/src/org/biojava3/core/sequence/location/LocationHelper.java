...
public class LocationHelper {
    ...
    /**
     * Used as a thin wrapper to the {@link #location(java.util.List, java.lang.String) } method to
     * bring the given location list together as a join (the default type)
     */
    ...
    /**
     * Builds a location from a List of locations; this can be circular or linear joins. The code
     * expects that these locations are in a sensible format.
     *
     * @param subLocations The list of locations to use to build the location. If given a list of size 1
     *        we will return that location.
    ...
    /**
     * Returns a location object which unlike the location constructors allows you to input reverse
     * coordinates and will convert these into the right location on the positive strand.
     */
    ...
    /**
     * Converts a location which defines the outer bounds of a circular location and splits it into the
     * required portions. Unlike any other location builder this allows you to express your input
     * location on the reverse strand
     *
     * @param location The location which currently expresses the outer bounds of a circular location.
     * @param length The length of the circular genomic unit
     * @return The circular location; can optionally return a normal non circular location if the one
    ...
    public static Location circularLocation(int start, int end, Strand strand, int length) {
        ...
        if (min > length) {
            throw new IllegalArgumentException("Cannot process a " + "location whose lowest coordinate is less than "
    ...
    /**
     * Used for scanning through a list of locations; assumes the locations given will have at least one
     * value otherwise we will get a null pointer
     */
    ...
    /**
     * Takes a point on a circular location and moves it left until it falls at the earliest possible
     * point that represents the same base.
     *
    ...
    /**
     * Works in a similar way to modulateCircularLocation but returns the number of complete passes over
     * a Sequence length a circular location makes i.e. if we have a sequence of length 10 and the
     * location 3..52 we make 4 complete passes through the genome to go from position 3 to position 52.
     */
    ...
    /**
     * Loops through the given list of locations and returns true if it looks like they represent a
     * circular location. Detection cannot happen if we do not have consistent accessions
     */
    ...
    /**
     * Scans a list of locations and returns true if all the given locations are linked to the same
     * sequence. A list of null accessioned locations is the same as a list where the accession is the
     * same
     *
    ...
    /**
     * Loops through the given list of locations and returns the consensus Strand class. If the class
     * switches then we will return an undefined strand
     */
    ...
    /**
     * This will attempt to find what the last point is and returns that position. If the location is
     * circular this will return the total length of the location and does not mean the maximum point on
     * the Sequence we may find the locations on
     */
