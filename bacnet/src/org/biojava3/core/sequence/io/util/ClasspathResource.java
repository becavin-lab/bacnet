...
/**
 * This object represents a classpath resource on the local system. It allows you to specify a
 * location and then extract the inputstream, reader or lines of the resource. We also support
 * GZiped files (so long as the resource ends with a .gz) and pre-caching of the data so we read
 * only once from the classpath and close that link down. This is useful if you want to keep IO
...
public class ClasspathResource {
     ...
     *
     * @param location Specified as <i>my/classpath/loc.txt</i>
     * @param preCache If set to true will cause the data to be copied to an in memory byte array and
     *        then an InputStream will be wrapped around that.
     */
    ...
    /**
     * Advanced constructor which lets you set the preCache variable and to force the type of file we
     * are decompressing. If this constructor is used we trust your call as to the file's compression
     * status.
     *
     * @param location Specified as <i>my/classpath/loc.txt</i>
     * @param preCache If set to true will cause the data to be copied to an in memory byte array and
    ...
    private InputStream createClasspathInputStream() {
        ...
        if (classpathIs == null) {
            throw new IllegalArgumentException("Location " + location + " resulted in a null InputStream");
        }
        if (preCache) {
    ...
    /**
     * Returns true if the location given ends with a .gz extension. No magic number investigation is
     * done.
     */
