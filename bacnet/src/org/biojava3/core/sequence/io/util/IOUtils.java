...
public class IOUtils {
    ...
    /**
     * Closes any Object which implements the interface {@link Closeable} and sending any error to the
     * logger but not forcing any explicit catching of stream errors.
     *
    ...
    /**
     * Takes in a reader and a processor, reads every line from the given file and then invokes the
     * processor. What you do with the lines is dependent on your processor.
     *
    ...
    /**
     * Delegates to {@link #getList(BufferedReader)} by wrapping the InputStream in a valid reader. No
     * encoding is mentioned so if you need anything more advanced then use the other version of this
     * method.
     *
     * @param is InputStream which is a text file
     * @return List of Strings representing the lines of the files
     * @throws ParserException Can throw this if the file is not a file or we cannot parse it
     */
    ...
    /**
     * Delegates to {@link #getList(InputStream)} by wrapping the File in a valid stream. No encoding is
     * mentioned so if you need anything more advanced then use the other version of this method. Since
     * this uses {@link #openFile(File)} this code can support GZipped and plain files.
     *
    ...
    /**
     * For a filename this code will check the extension of the file for a .gz extension. If it finds
     * one then the InputStream given back is a {@link GZIPInputStream}. Otherwise we return a normal
     * {@link FileInputStream}.
     *
     * @param file File which may or may not be GZipped
     * @return The final stream
     * @throws ParserException Can throw this if the file is not a file or we cannot open it for
     *         processing
     */
    ...
    /**
     * Closure interface used when working with {@link IOUtils#processReader(String)}. Each time a line
     * is encountered the object that implements this interface will be invoked.
     *
     * @author ayates
     */
