...
/**
 * Need to keep track of actual bytes read and take advantage of buffered reader performance. Took
 * java source for BufferedReader and added BytesRead functionality
 * 
 ...
 *         BufferedReaderBytesRead around any Reader whose read() operations may be costly, such as
 ...
 *         DataInputStream with an appropriate BufferedReaderBytesRead.
 *
 * @see FileReader
...
public class BufferedReaderBytesRead extends Reader {
    ...
    /**
     * Keep track of bytesread via ReadLine to account for CR-LF in the stream. Does not keep track of
     * position if use methods other than ReadLine. //TODO should override other methods and throw
     * exception or keep track of bytes read
     * 
     * @return
     */
    ...
    private int read1(char[] cbuf, int off, int len) throws IOException {
        if (nextChar >= nChars) {
             ...
             * activity, and if line feeds are not being skipped, do not bother to copy the characters into the
     ...
     * <code>{@link Reader#read(char[], int, int) read}</code> method of the <code>{@link Reader}</code>
     * class. As an additional convenience, it attempts to read as many characters as possible by
     * repeatedly invoking the <code>read</code> method of the underlying stream. This iterated
     ...
     * end-of-file, or
     *
     * <li>The <code>ready</code> method of the underlying stream returns <code>false</code>, indicating
     ...
     * end-of-file then this method returns <code>-1</code>. Otherwise this method returns the number of
     ...
     *
     * <p>
     * Ordinarily this method takes characters from this stream's character buffer, filling it from the
     * underlying stream as necessary. If, however, the buffer is empty, the mark is not valid, and the
     * requested length is at least as large as the buffer, then this method will read characters
    ...
    /**
     * Tells whether this stream is ready to be read. A buffered character stream is ready if the buffer
     * is not empty, or if the underlying character stream is ready.
     *
     * @exception IOException If an I/O error occurs
     */
    public boolean ready() throws IOException {
        synchronized (lock) {
            ...
            /*
             * If newline needs to be skipped and the next char to be read is a newline character, then just
             * skip it right away.
             */
            if (skipLF) {
                /*
