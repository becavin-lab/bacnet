 ...
 * is actually carried out in the {@link BitArrayWorker} instances. These are the objects that carry
 ...
 * <li>For real savings you must read the sequence in using your own Reader and a
 * {@link BitArrayWorker} instance</li>
 * </ul>
 *
 * @author ayates
 *
...
public class BitSequenceReader<C extends Compound> implements ProxySequenceReader<C> {
    ...
    public static abstract class BitArrayWorker<C extends Compound> {
        ...
        /**
         * This method should return the bit mask to be used to extract the bytes you are interested in
         * working with. See solid implementations on how to create these
         */
        ...
        /**
         * Should return the inverse information that {@link #generateCompoundsToIndex() } returns i.e. if
         * the Compound C returns 1 from compoundsToIndex then we should find that compound here in position
         * 1
         */
        protected abstract List<C> generateIndexToCompounds();

        /**
         * Returns what the value of a compound is in the backing bit storage i.e. in 2bit storage the value
         * 0 is encoded as 00 (in binary).
         */
        ...
        /**
         * Loops through the chars in a String and passes them onto {@link #setCompoundAt(char, int)}
         */
        ...
        public C getCompoundAt(int position) {
            ...
            if (masked > (compoundsPerDatatype() - 1)) {
                throw new IllegalStateException("Got a masked value of " + masked
                        + "; do not understand values greater than " + (compoundsPerDatatype() - 1));
            }
            return getIndexToCompoundsLookup().get(masked);
        }

        /**
         * Since bit encoding only supports a finite number of bases it is more than likely when processing
         * sequence you will encounter a compound which is not covered by the encoding e.g. N in a 2bit
         * sequence. You can override this to convert the unknown base into one you can process or store
         * locations of unknown bases for a level of post processing in your subclass.
         *
        ...
        protected byte processUnknownCompound(C compound, int position) throws IllegalStateException {
            throw new IllegalStateException("Do not know how to translate the compound " + compound + " to a "
                    + bitsPerCompound() + "bit representation");
        }

        /**
         * Returns a list of compounds the index position of which is used to translate from the byte
         * representation into a compound.
         */
        ...
        /**
         * Converting a biological index to the int which is used to store that position's data.
         *
         * <ul>
