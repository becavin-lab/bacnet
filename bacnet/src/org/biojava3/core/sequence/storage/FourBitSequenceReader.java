...
public class FourBitSequenceReader<C extends Compound> extends BitSequenceReader<C> {
    ...
    /**
     * A four bit per compound implementation of the bit array worker code. This version can handle upto
     * 16 compounds but this does mean that its ability to compress a normal sequence is halved
     * (compared to the 1/4 performance seen with the 2bit workers).
     *
    ...
    public static class FourBitArrayWorker<C extends Compound> extends BitArrayWorker<C> {
        ...
        private final static byte MASK =
                (byte) ((int) Math.pow(2, 0) | (int) Math.pow(2, 1) | (int) Math.pow(2, 2) | (int) Math.pow(2, 3));
        ...
        /**
         * Returns a Map which encodes the contents of CompoundSet. This version is case-insensitive i.e. C
         * and c both encode for the same position. We sort lexigraphically so if the compound set has not
         * changed then neither will this.
         */
