...
/**
 * This reader actually proxies onto multiple types of sequence in order to allow a number of
 * sequence objects to act as if they are one sequence. The code takes in any number of sequences,
 * records the minimum and maximum bounds each sequence covers with respect to 1 position indexing
 * and then binary searches these when a position is requested. Because of this 0 length Sequences
 * are excluded during construction.
 *
 * Performance is not as good as if you are using a flat sequence however the speed of lookup is
 * more than adaquate for most situations. Using the iterator gives the best performance as this
 * does not rely on the binary search mechanism instead iterating through each sequence in turn.
 *
 * @author ayates
...
public class JoiningSequenceReader<C extends Compound> implements ProxySequenceReader<C> {
    ...
    /**
     * Internal implementation flag and controls how we look for the right sub-sequence
     */
    ...
    /**
     * Allows creation of the store from Vargs Sequence<C> objects. CompoundSet defaults to the first
     * element of the array (assuming there are elements available during construction otherwise we will
    ...
    /**
     * Allows creation of the store from List<Sequence<C>>. CompoundSet defaults to the first element of
     * the List (assuming there are elements available during construction otherwise we will throw an
    ...
    /**
     * Scans through the sequence index arrays in linear time. Not the best performance but easier to
     * code
     */
    ...
    /**
     * Iterator implementation which attempts to move through the 2D structure attempting to skip onto
     * the next sequence as & when it is asked to
     */
