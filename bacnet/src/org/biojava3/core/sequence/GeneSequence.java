...
public class GeneSequence extends DNASequence {
    ...
    private final LinkedHashMap<String, TranscriptSequence> transcriptSequenceHashMap =
            new LinkedHashMap<String, TranscriptSequence>();
    private static final Logger log = Logger.getLogger(GeneSequence.class.getName());
    private final LinkedHashMap<String, IntronSequence> intronSequenceHashMap =
            new LinkedHashMap<String, IntronSequence>();
     ...
     * Two important concepts that is difficult to make everything flexible but still work. You can have
     * GFF features that only describe Exons or Exons/Introns or CDS regions and one or more
     * Transcriptions. You can have exon sequences but that does not imply transcription to the actual
     * protein.
     *
     * The GeneSequence will keep track of Exons and Introns but to get a Protein sequence you need to
     * start with a TranscriptSequence and then add CDS sequences.
     *
    ...
    /**
     * Once everything has been added to the gene sequence where you might have added exon sequences
     * only then you can infer the intron sequences and add them. You may also have the case where you
     * only added one or more TranscriptSequences and from that you can infer the exon sequences and
     * intron sequences. Currently not implement
     */
    public void addIntronsUsingExons() throws Exception {
        ...
        for (int i = 0; i < exonSequenceList.size() - 1; i++) {
            ...
            this.addIntron(new AccessionID(this.getAccession().getID() + "-" + "intron" + intronIndex),
                    exon1.getBioEnd() - shift, exon2.getBioBegin() + shift);
            intronIndex++;
        }
    ...
    /**
     * Try to give method clarity where you want a DNASequence coding in the 5' to 3' direction Returns
     * the DNASequence representative of the 5' and 3' reading based on strand
     * 
     * @return dna sequence
     */
