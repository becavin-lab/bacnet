...
/**
 * This class represents the storage container of a sequence stored in a fasta file where the
 * initial parsing of the file we store the offset and length of the sequence. When a call is made
 * to any method that needs sequence data then the file will be opened and the sequence loaded. This
 * class could be improved by using the hints or a some algorithm that indicates the sequence data
 * once loaded should stay loaded. Could keep track of the last time sequence data was loaded and
 * then after X amount of time clear the contents to free up memory.
 *
 *
...
public class SequenceFileProxyLoader<C extends Compound> implements ProxySequenceReader<C> {
    ...
    public SequenceFileProxyLoader(File file, SequenceParserInterface sequenceParser, long sequenceStartIndex,
            int sequenceLength, CompoundSet<C> compoundSet) {
    ...
    private boolean init() {
        try {
            ...
            throw new FileAccessError("Error accessing " + file + " offset=" + sequenceStartIndex + " sequenceLength="
