...
/**
 * Provides a way of separating us from the specific {@link IUPACTable} even though this is the only
...
public interface Table {
    ...
    /**
     * Returns true if the given compound could have been a start amino acid; this does not assert if
     * the codon that actually coded for the amino acid was a start codon. This is as accurate a call as
     * we can make with an {@link AminoAcidCompound}.
     */
    boolean isStart(AminoAcidCompound compound);

    /**
     * Instance of a Codon which is 3 {@link NucleotideCompound}s, its corresponding
     * {@link AminoAcidCompound} and if it is a start or stop codon. The object implements hashCode &
     * equals but according to the nucleotide compounds & not to the designation of it being a start,
     * stop & amino acid compound
     *
     * @author ayates
     *
     */
    ...
    public static class CaseInsensitiveTriplet {
        ...
        public String toString() {
            if (!stringSet) {
                stringify = getOne().getUpperedBase() + getTwo().getUpperedBase() + getThree().getUpperedBase();
            }
            return stringify;
        }
