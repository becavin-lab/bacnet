...
/**
 * Takes a {@link Sequence} of {@link NucleotideCompound} which should represent an RNA sequence
 * ({@link RNASequence} is good for this) and returns a list of {@link Sequence} which hold
 * {@link AminoAcidCompound}. The translator can also trim stop codons as well as changing any valid
...
public class RNAToAminoAcidTranslator extends AbstractCompoundTranslator<NucleotideCompound, AminoAcidCompound> {
    ...
    // Cheeky lookup which uses a hashing value; key is to switch to using this all
    // the time
    ...
    /**
     * Performs the core conversion of RNA to Peptide. It does this by walking a windowed version of the
     * given sequence. Any trailing DNA base pairs are ignored according to the specification of
    ...
    /**
     * Imperfect code. Checks the last amino acid to see if a codon could have translated a stop for it.
    ...
    /**
     * Indicates if we want to force exact translation of compounds or not i.e. those with internal N
     * RNA bases. This will cause a translation to an X amino acid
     */
