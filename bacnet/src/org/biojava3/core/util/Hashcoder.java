...
public class Hashcoder {
    ...
    /**
     * An initial value for a <code>hashCode</code>, to which we add contributions from fields. Using a
     ...
     *
     * Where result is the result of the previous calculation (at first this will be seed) and c is the
     * calculated int to add to result
     */
    ...
    /**
     * float support done via {@link Float#floatToIntBits(float)} which allows the encoding of a float
     * as an int. We can then hash as normal.
     */
    ...
    /**
     * double support which is done using the same techinque as float hashing except we convert to a
     * long not to an int.
     */
     ...
     *
     * If <code>o</code> is an array, then each element may be a primitive or a possibly-null object.
     */
