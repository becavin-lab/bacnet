 ...
 *
 * Created on May 26, 2010 Author: Mark Chapman
 */
...
/**
 * Static utility to easily share a thread pool for concurrent/parallel/lazy execution. To exit
 * cleanly, {@link #shutdown()} or {@link #shutdownAndAwaitTermination()} must be called after all
...
public class ConcurrencyTools {
    ...
    /**
     * Sets thread pool to any given {@link ThreadPoolExecutor} to allow use of an alternative execution
     * style.
     *
    ...
    /**
     * Closes the thread pool. Waits 1 minute for a clean exit; if necessary, waits another minute for
     * cancellation.
     */
    ...
    public static <T> Future<T> submit(Callable<T> task, String message) {
        // TODO log("Task " + (++tasks) + " submitted to shared thread pool. " +
        // message);
