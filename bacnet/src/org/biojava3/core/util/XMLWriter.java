...
public interface XMLWriter {
    ...
    /**
     * Open a new unqualified XML tag. This may also be used if you want to do namespace management
     * yourself, independantly of the XMLWriter
     *
    ...
    /**
     * Add an attribute to an element. This will throw an exception if it's not called immediately after
     * an <code>openTag</code> command.
     *
     * @param nsURI A URI for the namespace to use
    ...
    /**
     * Add an un-qualified attribute to an element. This will throw an exception if it's not called
     * immediately after an <code>openTag</code> command.
     *
    ...
    /**
     * Hints that a namespace is going to be used in a sub-tree. Use this method to avoid namespaces
     * that are used only in leaf-nodes of a tree being re-defined every time they are used. The
     * XMLWriter will generally try to use the suggested prefix for this namespace, but there is no
     * guarentee of this. In particular, if the namespace is already in use, the current prefix will
     * still be used. Similarly if the suggested prefix has already been used for another namespace, a
     * new one will be auto-generated.
     *
