...
public abstract class AbstractSequence<C extends Compound> implements Sequence<C> {
    ...
    /**
     * A ProxySequenceReader allows abstraction of both the storage of the sequence data and the
     * location of the sequence data. A variety of use cases are possible. A ProxySequenceReader that
     * knows the offset and of teh sequence in a large fasta file. A ProxySequenceReader that can pull
     * Sequence data from Uniprot, NCBI or a custom database. If the ProxySequecneReader implements
     * various interfaces then the sequence will set those interfaces so that calls to various methods
     * will be valid.
     *
     * @param proxyLoader
     * @param compoundSet
     ...
     * will gain additional interface inspection that allows external data sources with knowledge of
     * features for a sequence to be supported.
     * 
     * @param proxyLoader
     */
    ...
    /**
     * @param abstractSequence the parentSequence to set
     */
    public void setParentSequence(AbstractSequence<C> abstractSequence) {
        this.parentSequence = abstractSequence;
    }

    /**
     * Added support for the source of this sequence for GFF3 export If a sub sequence doesn't have
     * source then check for parent source
     * 
     * @return the source
     */
    ...
    /**
     * Method to help set the proper details for a feature as it relates to a sequence where the feature
     * needs to have a location on the sequence
     * 
     * @param bioStart
     * @param bioEnd
     * @param feature
    ...
    /**
     * Add a feature to this sequence. The feature will be added to the collection where the order is
     * start position and if more than one feature at the same start position then longest is added
     * first. This helps on doing feature layout for displaying features in SequenceFeaturePanel
     * 
     * @param feature
     */
    ...
    /**
     * Default case is to assume strand is positive because only CDSSequence can be either positive or
     * negative Strand.
     * 
     * @return
     */
