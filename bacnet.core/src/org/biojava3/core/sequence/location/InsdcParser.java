...
public class InsdcParser {
     ...
     */
    private static final Pattern SINGLE_LOCATION =
            Pattern.compile("\\A ([A-Za-z.0-9]*?) :? ([<>]?) (\\d+) \\Z", Pattern.COMMENTS);

    /**
     * Parses a location of the form Accession:1..4 (also supports the ^ format and undefined locations)
     */
    private static final Pattern RANGE_LOCATION =
            Pattern.compile("\\A ([A-Za-z.0-9]*?) :? ([<>]?) (\\d+) ([.^]+) ([<>]?) (\\d+) \\Z", Pattern.COMMENTS);
     ...
     *
     * @param reader The source of the data; assumes that end of the reader stream is the end of the
    ...
    protected Location parseRange(Matcher matcher, Strand strand) {
        ...
        Point start = generatePoint(matcher.group(3), matcher.group(2));
        Point end = generatePoint(matcher.group(6), matcher.group(5));
