...
/**
 * A collection of locations which are used whenever we work with INSDC; some of which could be
 * deprecated (from INSDC's point of view) yet appear in records.
 *
 * @author ayates
 */
public class InsdcLocations {
    ...
    public static class BondLocation extends AbstractLocation {
        ...
        protected final void assertLocation() {
            for (Location l : this) {
                ...
                Point end = l.getEnd();
                if (!start.equals(end)) {
                    throw new IllegalStateException("The start " + start + " is not equal to the end " + end
                            + ". bond locations must be a single " + "compound long");
                }
            }
        }
    }
    ...
    /**
     * Deprecated in INSDC yet still appears; equivalent to the order() directive except no 5' to 3'
     * ordering is defined. The constructor reflects this relationship and only allows the creation of
     * complex locations
     */
