...
public interface Location extends Iterable<Location>, Accessioned {
    ...
    /**
     * Basic location which is set to the minimum and maximum bounds of {@link Integer}. {@link Strand}
     * is set to {@link Strand#UNDEFINED}.
     */
    ...
    /**
     * Gives access to the sub locations for this location. However this does not return sub-locations
     * of sub-locations. For that functionality use {@link #getAllSubLocations()}.
     *
    ...
    /**
     * An extension to {@link #getSubLocations()} which returns sub-locations of sub-locations; this
     * will continue until it runs out of those locations.
     *
    ...
    /**
     * Returns true if the location is considered to be complex meaning the location is actually
     * composed of sub-locations.
     */
    boolean isComplex();
    ...
    /**
     * Returns true if the position is meant to represent a point between two points such as 78^79. Only
     * valid if start and stop are next to each other.
     */
    ...
    /**
     * Will return a SequenceReader object which offers a view of all resolved locations i.e. those
     * locations which are not complex and define the true Sequence represented
     *
     * @param &lt;C&gt; The type of compound to use
     * @param sequence The sequence object to work with
        ...
        public static Location circularLocation(int start, int end, Strand strand, int length) {
            ...
            if (min > length) {
                throw new IllegalArgumentException("Cannot process a "
                        + "location whose lowest coordinate is less than " + "the given length " + length);
            }
