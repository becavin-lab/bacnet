 ...
 * 
 *         Here I modified the original Chromosome class to one which include: coding and noncoding
 *         regions + proteins
        ...
        if (file.exists()) {
            ...
            for (int i = 0; i < suppData.length; i++) {
                ...
                if (suppData[i][3].contains("Operon")) {
                    Operon operon =
                            Operon.convertDNASequenceOperon(ArrayUtils.getRow(suppData, i), this.getParentGenome());
                    operons.add(operon);
                } else if (suppData[i][3].contains("Term")) {
                    String[] infos = ArrayUtils.getRow(suppData, i);
                    Sequence element =
                            new Sequence("Term_" + infos[4], Integer.parseInt(infos[0]), Integer.parseInt(infos[1]));
