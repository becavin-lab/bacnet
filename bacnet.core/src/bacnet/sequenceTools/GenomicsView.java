...
*/
		...
		for (int i = 0; i < bioCondsArray[0].length; i++) {
			...
			col2.setLabelProvider(new CellLabelProvider() {
				...
				public void update(ViewerCell cell) {
					...
					} else if (colName.equals("Most recent assignation (cgMLST)")||colName.equals("Most recent assignation")){
	...
	/**
	 * Load SVG figure of phylogeny and highlight selected strains
	 */
	...
	public String getPhyloFigure(ArrayList<String> genomeNames) {
		...
		for (String genome : genomeNames) {
			...
			if (lineAttribute.contains("matrix")) { //old method, for svg generated by FigTree v1.4.4 for example
				//int indexOfstyle = lineAttribute.indexOf("style");
				//int lengthStyle = "style=\"".length();
				int posToADD = indexOfLine + lineAttribute.length();
			...
			} else if (lineAttribute.contains("translate")) { // new method, for svg generated by Iroki.net for example	
	...
	public void widgetSelected(SelectionEvent e) {
		...
		if (e.getSource() == btnSavePng) {
			...
			try {
				...
				SaveFileUtils.saveFile(Database.getInstance().getSpecies()+"_Phylogenomic_Tree.png", tempPNGFile, "PNG image file", partService,
						shell);
			} catch (IOException e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
