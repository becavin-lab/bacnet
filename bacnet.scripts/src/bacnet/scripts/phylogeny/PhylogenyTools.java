...
public class PhylogenyTools {
	...
	public static String ALLBACTERIA_XML_1 = Database.getANALYSIS_PATH()
			+ "/Egd-e Annotation/1565_52loci_nucleotides.xml";
	...
	/**
	 * If a node of the Phylogeny phy, is not a key of infoNodes Map, delete the
	 * tree <br>
	...
	/**
	 * From PhyloXMl of all bacteria, get only the genome which have a NCBI genome
	 * id
	 * 
	 * @return
	 */
...
//	     System.out.println(phy.getNumberOfExternalNodes()+" nodes");
	...
	public static void createInfoNodes() {
		...
		// this array give link between NCBI genome ID(for genome info, Blast results),
		// Rfam genome ID (for ncRNA), and IMG_taxon ID (for phylogeny)
		...
		for (int i = 0; i < result.length; i++) {
			...
			// search for each row of results, the corresponding Rfam Id in
			// RfamElement.RFAM_GENOME array
				...
				if (genomeName.equals(array[k][4])) {
					...
					if (infoNodes.containsKey(array[k][8])) {
						...
						try {
							...
							System.out.println(row[0] + " " + row[5] + "  " + row[4] + " " + rowResult[4] + "  " + "  "
									+ rowResult[5] + " " + rowResult[0]);
						}
