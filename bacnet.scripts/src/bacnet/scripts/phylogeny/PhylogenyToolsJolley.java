...
/**
 * This list of function is useful to manipulate Newick files given by Jolley in
 * May 2012, from its publication Jolley et al. Microb. 2012<br>
 * It contains 1565 nodes, each bacteria=node has an ID. The link between these
 * ID and an NCBI ID (i.e. NC32010.1) is given by the table at
...
public class PhylogenyToolsJolley {
	...
	public static String ALLBACTERIA_NWK_1 = Database.getANALYSIS_PATH()
			+ "/Egd-e Annotation/1565_52loci_nucleotides_labels.nwk";
	public static String ALLBACTERIA_NWK_2 = Database.getANALYSIS_PATH()
			+ "/Egd-e Annotation/1565_52loci_protein_labels.nwk";
	public static String ALLFIRMICUTES_AVAILABLE = Database.getANALYSIS_PATH()
			+ "/Egd-e Annotation/All-Firmicutes-Availables.txt";
	...
	/**
	 * If a node of the Phylogeny phy, is include in listIDs, hide it by setting the
	 * name to void <br>
	 * 
	 * @param phy
	 * @param infoNodes
	...
	/**
	 * Get a Phylogeny where nodeName contain ID from Jolley and add all taxonomy
			...
			if (!name.equals("")) {
				...
				if (jolleyIDtoInfo.containsKey(id)) {
					if (include(jolleyIDtoInfo.get(id))) {
						node.setName(jolleyIDtoInfo.get(id)[6] + " " + jolleyIDtoInfo.get(id)[1]); // species + isolate
																									// + NCBI id
						Taxonomy t1 = new Taxonomy();
						t1.setScientificName(jolleyIDtoInfo.get(id)[11]); // species..class
						t1.setAuthority(jolleyIDtoInfo.get(id)[13] + " - " + jolleyIDtoInfo.get(id)[14]); // sequencing
																											// center
