...
//		createGenomeDB();
	 ...
	 * <li>all ASrna on 3 frames (300bp long) lists of Genes, ID=lmo, directly taken
	 * from NCBI lists of "new"Gene, ID=rastlmo, from the analysis of annotation I
	 * just did using RAST software lists of Srna, ID=rli, rliM, rliP, we don't know
	 * where is the begining so we have 3 frame for each rli, we start at the first
	 * start codon, plus we cut when we found stop codon and peptide smaller than
	 * 5aa. lists of ASrna, ID=anti, antiM, antiP, we don't know where is the
	 * begining so we have 3 frame for each asRNA, we start at the first start
	 * codon, plus we cut when we found stop codon and peptide smaller than 5aa.
	 * 
	 */
	 ...
	 * We found the correct frame so we are sure to have the start codon of the gene
	 * at the exact position of NCBI annotation We start at the first start codon,
	 * then if stop codon are found in the UTR region we cut in different peptides,
	 * remove ones < 5aa, and paste to the last part of the UTR the whole gene amino
	 * acide sequence all gene, ID=lmo_n n being the id of the UTR peptide extracted
	 */
	 ...
	 * The small peptides include in the list are the one only of size more than
	 * 5aa, and surrounded by at least one R.<br>
	 ...
	 * <li>frame 6 = EGDe reverse complement genome 1bp on the right,
	 * ID=genomeComplP <br>
	 */
	...
	/**
	 * From an amino acid sequence of a genome create final fasta file by cutting at
	 * each Start, Stop and R positions
	 * 
	 * @param seqAA
	 * @param prefix
	 * @param fileName
	...
	/**
	 * Create database of all peptide from a start codon (M=ATG, L=TTG, V=GTG), to
	 * the next stop codon or R
	 */
	...
	/**
	 * Divide TISDB into sequence starting with a M (TISMDB), and other not starting
	 * with an M (TISnoMDB).
	 */
	...
	/**
	 * Given a genome sequence, and a list of position of start codon, update a list
	 * of peptides accordingly<br>
	 * The new peptide, are the peptide from a start codon to the next R or stop
	 * codon
	 * 
	 * @param sequence
			...
			if (index == -1 && index == -1) {
				...
				if (subSeqAA.length() > (LENGTH_CUTOFF - 1)) {
					if (subSeqAA.length() < cutoff) {
						...
						/*
						 * If we are on minus strand working with reverse complement sequence the real
						 * position on the genome is : genomeLength-position
						 */
...
//		String seqAAComplete =seqAA;
	 ...
	 * <li>use <code>http://web.expasy.org/compute_pi/</code> to calculate mass and
	 * isoelectric point
