...
public class GenomeNCBIFolderTools {
	 ...
	 * <fi> download them: fna and gff file only
	 * <fi> create a folder for each
	 * <fi> verify MD5 checksum
	 * <fi> unzip files
	 * 
	 * The strain name should be in the column with the name: strain_name
	 * The folder name for the genome to be donwload in should be in the column with the name: folder_name	
	 * The ftp path should be found in the column with name : ftp_path
	 * The id of the assembly should be found in the column with name : assembly_accession
	 * 
	 * @param tableGenomes
	 * @param pathGenomes
	 */
...
//			System.out.println(genomeID);
	 ...
	 * folderFTP+"_assembly_report.txt"
	 * folderFTP+"_assembly_stats.txt"
	 * folderFTP+"_genomic.fna.gz"
	 * folderFTP+"_genomic.gff.gz"
	 * "md5checksums.txt"
	 * 
	 * @param folderFTP
	 * @param path
...
//			System.out.println(realmd5faa+" "+md5faa+" "+namefaa);
	...
	/**
	 * In NCBI genome folder remove everything which is not useful for my analysis<br>
		...
		for(File genomeFolder : file.listFiles()){
			if(genomeFolder.isDirectory()){
				...
				for(File fileTemp : genomeFolder.listFiles()){
					...
					if(extension.equals(".asn")) fileTemp.delete();
					else if(extension.equals(".gbk")) fileTemp.delete();
					else if(extension.equals(".tab")) fileTemp.delete();
					else if(extension.equals(".rpt")) fileTemp.delete();
					else if(extension.equals(".gbs")) fileTemp.delete();
					else if(extension.equals(".gbk")) fileTemp.delete();
					else if(extension.equals(".Glimmer3")) fileTemp.delete();
					else if(extension.equals(".val")) fileTemp.delete();
					else if(extension.equals(".rnt")) fileTemp.delete();

					if(fileTemp.getAbsolutePath().contains(".GeneMark")) fileTemp.delete();
					if(fileTemp.getAbsolutePath().contains(".Prodigal-2.50")) fileTemp.delete();
				}
			}
		}
	}
	...
	/**
	 * Go through all folders of Genome directory and extract all TGZ
	 * contained in .faa .fna .ffn .frn .gff
	 * 
	 */
		...
		for(String geno : genomes){
			...
			for(File file : filesTGZ){
				...
				for(String extension : FastaFileReader.allfileExt){
					if(name.contains(extension)) uncompress = true;
				}
				// test if it is a gff file
				if(name.contains(GFFNCBIReader.EXTENSION)) uncompress = true;

				// if it is already uncompress and we don't want to renew it, we do nothing (uncompress=false)
				if(alreadyUncompress && !renew) uncompress = false;  
