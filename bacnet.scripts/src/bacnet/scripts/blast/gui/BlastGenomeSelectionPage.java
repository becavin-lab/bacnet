...
public class BlastGenomeSelectionPage extends WizardPage implements SelectionListener{
	...
	public void createControl(Composite parent) {
		try {
			...
			listBac.setItems(GenomeNCBITools.getCompleteGenome(GenomeNCBITools.getBacillusGenome()).toArray(new String[0]));
			...
			listNonMono.setItems(GenomeNCBITools.getCompleteGenome(GenomeNCBITools.getListeriaNonMonoGenome()).toArray(new String[0]));
			...
			listNonMono2.setItems(GenomeNCBITools.getWGS(GenomeNCBITools.getListeriaNonMonoGenome()).toArray(new String[0]));
			...
			listOther.setItems(GenomeNCBITools.getCompleteGenome(GenomeNCBITools.getNotListeriaAndBacillusGenome()).toArray(new String[0]));
			...
			listOther2.setItems(GenomeNCBITools.getWGS(GenomeNCBITools.getNotListeriaAndBacillusGenome()).toArray(new String[0]));
		}catch (IOException e) {
	...
	public ArrayList<String> getSelectedGenomes(){
		...
		for(String genome:listMono.getSelection()) genomes.add(genome);
		for(String genome:listMono2.getSelection()) genomes.add(genome);
		for(String genome:listNonMono.getSelection()) genomes.add(genome);
		for(String genome:listNonMono2.getSelection()) genomes.add(genome);
		for(String genome:listBac.getSelection()) genomes.add(genome);
		for(String genome:listBac2.getSelection()) genomes.add(genome);
		for(String genome:listOther.getSelection()) genomes.add(genome);
		for(String genome:listOther2.getSelection()) genomes.add(genome);
	...
	public void widgetSelected(SelectionEvent e) {
		if(e.getSource()==btnMono){
			if(btnMono.getSelection()) listMono.selectAll();
			else listMono.deselectAll();
		}else if(e.getSource()==btnMono2){
			if(btnMono2.getSelection()) listMono2.selectAll();
			else listMono2.deselectAll();
		}else if(e.getSource()==btnNonMono){
			if(btnNonMono.getSelection()) listNonMono.selectAll();
			else listNonMono.deselectAll();
		}else if(e.getSource()==btnNonMono2){
			if(btnNonMono2.getSelection()) listNonMono2.selectAll();
			else listNonMono2.deselectAll();
		}else if(e.getSource()==btnBac){
			if(btnBac.getSelection()) listBac.selectAll();
			else listBac.deselectAll();
		}else if(e.getSource()==btnBac2){
			if(btnBac2.getSelection()) listBac2.selectAll();
			else listBac2.deselectAll();
		}else if(e.getSource()==btnOther){
			if(btnOther.getSelection()) listOther.selectAll();
			else listOther.deselectAll();
		}else if(e.getSource()==btnOther2){
			if(btnOther2.getSelection()) listOther2.selectAll();
			else listOther2.deselectAll();
		}
	}
