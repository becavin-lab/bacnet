...
public class PredaTerm {
    ...
    // changing variables within reasonable limitis has little effect on p(A) prediction
                ...
                for (int p = 800 - up1; p < SeqTAB[i][1].length() - 800 - up1; p++) {
                    ...
                    for (int l = 0; l < up1 + down1; l++) {
                        switch (SeqTAB[i][1].charAt(p + l)) {
                            case 'A': {
                                score = (double) score + PSSM[l + (500 - up1)][0];
                                break;
                            }
                            case 'T': {
                                score = (double) score + PSSM[l + (500 - up1)][1];
                                break;
                            }
                            case 'C': {
                                score = (double) score + PSSM[l + (500 - up1)][2];
                                break;
                            }
                            case 'G': {
                                score = (double) score + PSSM[l + (500 - up1)][3];
                                break;
                            }
                            // uncomment this line to CONSIDER AMBIGUITY CHARACTERS as position mean
                            // case 'N': { score = (double) score + (pssm1[l][0]+pssm1[l][1]+pssm1[l][2]+pssm1[l][3])/4
                            // ;
                        }
                    }
                    big[matrix] = score;
                    ...
                    for (int l = 0; l < up2 + down2; l++) {
                        switch (SeqTAB[i][1].charAt(p + l)) {
                            case 'A': {
                                score = (double) score + PSSM[l + (500 - up2)][0];
                                break;
                            }
                            case 'T': {
                                score = (double) score + PSSM[l + (500 - up2)][1];
                                break;
                            }
                            case 'C': {
                                score = (double) score + PSSM[l + (500 - up2)][2];
                                break;
                            }
                            case 'G': {
                                score = (double) score + PSSM[l + (500 - up2)][3];
                                break;
                            }
                            // uncomment this line to CONSIDER AMBIGUITY CHARACTERS as position mean
                            // case 'N': { score = (double) score + (pssm1[l][0]+pssm1[l][1]+pssm1[l][2]+pssm1[l][3])/4
    ...
    private static int[] PredictSJ(String[][] SequenceTAB) throws IOException {
        ...
        for (int i = 0; i != SequenceTAB.length; i++) {
            ...
            while (AG.hasNext()) {
                ...
                // POLYNOMIAL scoring function (comment-out previous line if used and change SJthreshold to 125)
                // score = seg_len + seg_len* constant *Math.pow(((double)char_counter/seg_len - 0.55), 3) ;
                if (score > best) {
                    best_end = endpos;
                    best = score;
                }
