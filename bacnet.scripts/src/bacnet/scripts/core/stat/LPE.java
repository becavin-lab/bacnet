...
public class LPE {
    ...
    public static void testLPE() {
        try {
            ...
            // ExpressionMatrix.readFromFile(Project.getProjectFolder()+File.separator+"LPEdata1.txt", true);
            // ExpressionMatrix data2 =
            // ExpressionMatrix.readFromFile(Project.getProjectFolder()+File.separator+"LPEdata2.txt", true);
            // ExpressionMatrix resultLPE =
            // ExpressionMatrix.readFromFile(Project.getProjectFolder()+File.separator+"LPEresult.txt", true);
            ...
            //
            // double sigma_pooled = Math.sqrt((Math.PI/2)*lpe1.value(median1)/3 + lpe2.value(median2)/3);
            // result.setValue(sigma_pooled,i,11);
            // result.setValue(run(x, y, lpe1, lpe2),i,12);
            // }
            //
            // result.saveTab(Project.getProjectFolder()+File.separator+"LPEresultChris.txt", "gene");
            // ExpressionMatrix diff = ExpressionMatrixStat.minus(result, resultLPE);
            // diff.saveTab(Project.getProjectFolder()+File.separator+"LPEresultDiff.txt", "gene");
            // // ScatterPlotView.displayMatrix(amR, "A vs M");
            //

            ExpressionMatrix leyMatrix =
                    ExpressionMatrix.loadTab(Database.getInstance().getPath() + "/Leypreprocess 1.txt", true);
            // leyMatrix = ExpressionMatrixStat.log2(leyMatrix);



            // ExpressionMatrix leyMatrix = ExpressionMatrix.readFromFile(Project.getProjectFolder()+"/Ley
            // dataset.txt", true);
    ...
    /**
     * For each comparison calculate the LPE estimate, by creating a matrix of all expression values and
     * estimates LPE using each of these matrices
     * 
     * @param estimateLPE
     * @param bioCondsTemp
     */
    ...
    public static double calcP(double[] vector1, double[] vector2, PolynomialSplineFunction lpe1,
        ...
        try {
            ...
            if (sigma_pooled > 0) {
                sigma_pooled = Math.sqrt(Math.PI / 2 * sigma_pooled);
                // evalue p-value with a normal distribution of mean = 0 and stdDev = sigma_pooled
