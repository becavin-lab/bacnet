...
/**
 * Different methods to curate and parse the different data for Listeriomics website <br><br>
 * 
...
//		LT.calculateComparison("A-GEOD-11577", "E-GEOD-26690");
	 ...
	 * WARNING: No annotation information is allowed, only real values
	 * @param dataPath
	 * @param comparison true if it is an ExpressionMatrix for a comparison, if no LOGFC column are found in a row, it will delete the row (gain speed and space)
	 * @return
	 */
	public static ExpressionMatrix curateMatrix(String dataPath,boolean comparison){
		...
		/*
		 * go through array and remove duplicates by calculating mean of each column for every duplicate rowname
		 */
		...
		for(String rowName : matrix.getRowNames().keySet()){
			for(int j=1;j<array[0].length;j++){
				...
				sum = sum/count;
				if(count==0) sum = OmicsData.MISSING_VALUE;
	 ...
	 * From the web^page we create the table, curated it and convert it to HTML
	 *  Read DATA_TABLE and create an HTML table from it to ease accession to ArrayExpress info
	 */
	public static void 	createHTMLPageSummary(){
		...
		array = ArrayUtils.deleteColumn(array, array[0].length-1);
		TabDelimitedTableReader.saveInHTML(array, Database.getInstance().getPath()+"ListTranscriptomes.html","Listeria Transcriptomics data");
	}
