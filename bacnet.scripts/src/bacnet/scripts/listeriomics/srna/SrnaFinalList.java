...
public class SrnaFinalList {
	...
	/**
	 * Read the different :list from different publication and combine all of them
	 * into an Srna final list<br>
	...
	public static void createFinalList() {
		...
		/*
		 * add the list of sRNA from Oliver which have been modified, for each of the
		 * rli not in the other lists we align on EGDe, and create a new table with EGDe
		 * position of each sRNA Each new Srnas strand is curated by hand directly in
		 * the table Thanks to : Srna10403SOliver.findEGDePosition();
		 * 
		...
		for (Srna sRNA : toledo)
			sRNAsrawData.add(sRNA.getName());
		for (Srna sRNA : mraheil)
			sRNAsrawData.add(sRNA.getName());
		for (Srna sRNA : wurtzel)
			sRNAsrawData.add(sRNA.getName());
		for (Srna sRNA : rfam)
			sRNAsrawData.add(sRNA.getName());
		for (Srna sRNA : oliverEGDe)
			sRNAsrawData.add(sRNA.getName());
	...
	// /**
	// * Read all sRNA in Oliver table and extract all sRNA which are not present in
	// EGDe lists
	// * @throws IOException
	// */
	...
	// if(sRNA.getName().equals(sRNA2.getName()) ||
	// sRNA2.getSynonym().contains(sRNA.getName())){
	// found = true;
	...
	// XMLUtils.encodeToFile(sRNA,
	// Srna.PATHXML_10403S+sRNA.getId()+"-"+sRNA.getName()+".xml");
	// }
	// }

	/**
	 * Add the list of Srna contained in lst2 to the HashMap listSrnas<br>
	 * These function use the method sRNA1.isEqual(sRNA2, 40) to detect if two sRNAs
	 * are the same or not
	 * 
	 * @param listSrnas
	 * @param list2
	 ...
	 * @throws IOException
	 */
	private static HashMap<String, ArrayList<Srna>> curateList(HashMap<String, ArrayList<Srna>> listSrnas)
			throws IOException {
		String[][] curateTable = TabDelimitedTableReader
				.read(SrnaTables.PATH + "InfoForTableCuration/sRNA summaryTable.txt");
		...
		for (int i = 0; i < curateTable.length; i++) {
			if (listSrnas.containsKey(curateTable[i][4])) {
				...
				// link name of the sRNA to the reference Srna we need to use as a reference for
				// position
				nameToRefandType.put(curateTable[i][1],
						curateTable[i][2] + ";" + curateTable[i][3] + ";" + curateTable[i][0]);
			} else {
				System.err.println(curateTable[i][4]);
			}
		}

		// final step: put the sRNA selected as a reference in the first position in the
		// list
		...
		for (String name : listSrnasTemp.keySet()) {
			...
			// the first element should be the one to use for reference (position, strand,
			// type, etc)
			for (Srna sRNA : sRNAs) {
	...
	/**
	 * Combine all List of sRNA regrouped by "same center" into a general List of
	 * Srna <br>
	 * Use sRNA reference.txt <br>
	...
	private static ArrayList<Srna> createTotalList(HashMap<String, ArrayList<Srna>> listSrnas) {
		...
		for (String key : listSrnas.keySet()) {
			...
			for (Srna sRNATemp : listRNA) {
				...
				if (sRNA.getFeatures().containsKey("name (" + foundIn + ")")) {
					...
					for (String attribute : sRNATemp.getFeatures().keySet()) {
						sRNA.getFeatures().put(attribute.replaceAll("\\)", "-2\\)"),
								sRNATemp.getFeatures().get(attribute));
					}
					// update found in
			...
			// create ref name
			String[][] reference = TabDelimitedTableReader
					.read(SrnaTables.PATH + "InfoForTableCuration/sRNA reference.txt");
	...
	/**
	 * Given a list of Srna, this function create a Srna which have the right fixe
	 * attributes
	 * 
	 * @param listRNA
	 * @return
	...
	private static Srna createSrna(ArrayList<Srna> listRNA, String name) {
		...
		if (from == OmicsData.MISSING_VALUE) {
			from = to - (Sequence.DEFAULT_LENGTH - 1);
		} else if (to == OmicsData.MISSING_VALUE) {
			if (listRNA.get(0).isStrand()) {
				to = from + (Sequence.DEFAULT_LENGTH - 1);
			} else {
				to = from;
				from = from - (Sequence.DEFAULT_LENGTH - 1);
			}
		} else if (from > to) {
			int temp = from;
			from = to;
	...
	/**
	 * Add has a final step the information coming from oliver et al. 2009
	 * information in 10403S strain
	 * 
	 * @param finalList
	 * @param oliver
	 */
		...
		for (Srna sRNA : finalList) {
			for (Srna sRNAOliver : oliver) {
				if (sRNA.getName().equals(sRNAOliver.getName())) {
					...
					// update found in
					if (!sRNA.getFoundIn().contains(foundIn))
						sRNA.getFoundIn().add(foundIn);
				}
			}
		}
	...
	private static void displayListSrnas(HashMap<String, ArrayList<Srna>> listSrnas) {
		...
		for (String key : listSrnas.keySet()) {
			...
			for (Srna sRNA : listSrnas.get(key)) {
				ret += sRNA.getName() + " " + sRNA.getBegin() + " " + sRNA.getEnd() + " " + sRNA.isStrand() + " : "
						+ sRNA.getFoundIn().get(0) + " : " + sRNA.getTypeSrna() + "\t";
			}
			text += ret + "\n";
	 ...
	 * WARNING: GenomeConversion has to be run before! So we can access to the Srna
	 * through <code>Genome.loadEgdeGenome()</code>;
	 */
	public static void createSummaryTables() {
		Genome genome = Genome.loadEgdeGenome();
		ArrayList<String> finalList = new ArrayList<String>();
		String[] headersVector = { "Id", "Name", "From", "To", "Strand", "Length", "Synonim", "TSS",
				"First described in", "Also described in" };
		String header = "";
		...
		for (int i = 0; i < Srna.getSrnaOrder().size(); i++) {
			...
			String wurtzel = "";
			if (sRNA.getFoundInText().contains("Wurtzel et al. 2012"))
				wurtzel = "Yes";
	 ...
	 * Copy each .png image from UNAFold folder to
	 * <code>Srna.PATH_SEC_STRUCTURE</code>
	 */
		...
		for (File file : previousPath.listFiles()) {
			if (file.getAbsolutePath().contains(".png")) {
				try {
					...
					System.out.println(
							path.getAbsolutePath() + File.separator + FileUtils.removePath(file.getAbsolutePath()));
					FileUtils.copy(file.getAbsolutePath(),
							path.getAbsolutePath() + File.separator + FileUtils.removePath(file.getAbsolutePath()));
				} catch (IOException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
