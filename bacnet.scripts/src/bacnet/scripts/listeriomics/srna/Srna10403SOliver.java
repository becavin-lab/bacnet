...
public class Srna10403SOliver {
	...
	/**
	 * Get all Srna from Oliver et al. 2009 list, get Sequence in <code>10403S pseudo_chromosome</code><br>
	...
	public static void findEGDePosition(){
		...
		for(Srna sRNA : sRNAsTemp){
			if(sRNA.getBegin()==ExpressionMatrix.MISSING_VALUE || sRNA.getEnd()==ExpressionMatrix.MISSING_VALUE){
	...
	public static void runBlast(ArrayList<Srna> sRNAs){
		...
		for(Srna sRNA : sRNAs){
			String sequence = pseudoChromo10403S.getChromosomes().get(0).getSequenceAsString(sRNA.getBegin(), sRNA.getEnd(), Strand.POSITIVE);
			// save query sequence
	 ...
	 * It will be used by Blast afterwards<br><br>
	 * 
	...
	public static void summarizeResultsInTable(ArrayList<Srna> sRNAs) throws IOException{
		...
		String[] headers = {"Srna","EGD-e identites","EGD-e begin","EGD-e end","10403S identities","10403S begin","10403S end","10403S Average GEI","Delta_sigB Average GEI","FC range (10403S/Delta_sigB)"};
		...
		for(int i=0;i<sRNAs.size();i++){
			...
			for(int j=1;j<results[0].length;j++)	results[i+1][j] = "0";
			...
			for(BlastResult blt : results){
				...
				if(resultsTree.containsKey(blt.hit_accession)){
					...
					if(evaluePrevious > evalue){
						String[] result = {blt.eValue+"",blt.identities+"",blt.begin+"",blt.end+"",blt.strand+""};
