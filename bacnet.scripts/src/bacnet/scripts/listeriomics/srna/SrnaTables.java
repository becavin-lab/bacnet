...
/**
 * Include a list of Methods to read every Table containing Small RNAs in Listeria<br>
 * For each it reads the <code>Table</code> and create a <code>List</code> of <code>Srna</code>
 * 
 * @author UIBC
 *
 */
public class SrnaTables {
	...
	//public static String PATH = Database.getPATH()+"Genome"+File.separator+"sRNA"+File.separator+"RawData Tables"+File.separator;
	...
	public static ArrayList<Srna> getToledoInfo(){
		...
		for(int i=1;i<array.length;i++){
			Srna sRNA = new Srna(array[i][1],Integer.parseInt(array[i][2]),Integer.parseInt(array[i][3]),array[i][4].charAt(0));
		...
		for(int i=1;i<array.length;i++){
			...
			if(!sRNA.isStrand()) sRNA.setEnd(sRNA.getBegin()-sRNA.getLength());
			else sRNA.setEnd(sRNA.getBegin()+sRNA.getLength());
		...
		for(int i=1;i<array.length;i++){
			...
			sRNA.getFoundIn().add("Toledo-Arana et al. 2009");
			sRNA.setId(array[i][0]);
			if(sRNA.getEnd()==ExpressionMatrix.MISSING_VALUE) sRNA.setLength((int)ExpressionMatrix.MISSING_VALUE);
			else sRNA.setLength(Math.abs(sRNA.getBegin()-sRNA.getEnd()));
			sRNA.getFeatures().put("feature (Toledo-Arana et al. 2009)", array[i][5]);
			sRNA.getFeatures().put("note (Toledo-Arana et al. 2009)", array[i][6]);
			sRNA.getFeatures().put("ref (Toledo-Arana et al. 2009)", array[i][7]);
			sRNAs.add(sRNA);
	...
	public static ArrayList<Srna> getOliverInfoEGDePosition(){
		...
		System.err.println("Oliver sRNA "+array.length);
		for(int i=1;i<array.length;i++){
			Srna sRNA = new Srna(array[i][0],Integer.parseInt(array[i][1]),Integer.parseInt(array[i][2]),array[i][3].charAt(0));
			sRNA.setTypeSrna(TypeSrna.Srna);
			sRNA.setId(array[i][0]);
			sRNA.getFoundIn().add("Oliver et al. 2009");
	...
	public static ArrayList<Srna> getWurtzelInfo(){
		...
		for(int i=1;i<array.length;i++){
			Srna sRNA = new Srna(array[i][0],Integer.parseInt(array[i][1]),ExpressionMatrix.MISSING_VALUE,array[i][2].charAt(0));
			sRNA.setTypeSrna(TypeSrna.ASrna);
			sRNA.getFoundIn().add("Wurtzel et al. 2012");
			sRNA.setLength(ExpressionMatrix.MISSING_VALUE);
