...
//				SrnaAlignment.summarizeResultsInTable10403S();
		...
		/*
		 * We have to run in runPostInit(){ SrnaAlignment.createConservationTables(); }
...
//		for(Srna sRNA : genome.getChromosomes().get(0).getsRNAs().values()) sRNAs.add(sRNA);
	...
	/**
	 * Run a Blast for each sRNA (found in Oliver et al. in 10403S genome) on a list
	 * of Genomes given in tempDatabase
	 */
	...
	public static void multiSpeciesAlign(ArrayList<Srna> sRNAs) {
		...
		for (int i = 0; i < sRNAs.size(); i++) {
			...
			try {
				...
				String blastResultHTML = Database.getTEMP_PATH() + File.separator
						+ FileUtils.removeExtensionAndPath(blastResult);
...
//					}
	...
	/**
	 * Read all the XML results of Blast and create two PhyloXML files and a sRNA
	 * conservation table<br>
	 * 
	 * @throws IOException
	 */
	...
	public static String[][] summarizeResultsInTable(ArrayList<Srna> sRNAs) {
		...
		for (int i = 0; i < sRNAs.size(); i++) {
			...
			for (int j = 1; j < table[0].length; j++)
				table[i + 1][j] = "0";
			// read blast result
			String[][] bltResults = TabDelimitedTableReader
					.read(Database.getTEMP_PATH() + sRNA.getName().replaceAll("/", "") + ".txt");
...
//			FileUtils.saveText(phy.toPhyloXML(0),WebUtils.WEBPATH+"sRNAs"+File.separator+"PhyloXML"+File.separator+"PhyloXMLAll_"+sRNA.getName().replaceAll("/", "")+".xml");
	...
	/**
	 * Add Srna to any Genome by reading BlastResults and extracting the lower
	 * e-value result<br>
	 * Srna with an identities less than 25% are not kept!!!<br>
	 * 
	 * This method has to be run after <code>multiSpeciesAlign()</code> because it
	 * will uses the Blast Result files in Temp folder
	 * 
	 * @param genome
	 */
	...
	/**
	 * Read <code>Srna.PATH_CONSERVATION</code> table and display it in HeatMapView
	 * <br>
