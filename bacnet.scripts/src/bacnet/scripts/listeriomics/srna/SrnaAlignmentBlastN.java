...
public class SrnaAlignmentBlastN {
    ...
    public static void run() {
        ...
        /*
         * We have to run in runPostInit(){ SrnaAlignment.createConservationTables(); } to create table of
         * conservation and save them in html format!
         * 
         */
    }
    ...
    /**
     * Run a Blast for each sRNA (found in Oliver et al. in 10403S genome) on a list of Genomes given in
     * tempDatabase
     */
    ...
    public static void summarizeResultsInTable10403S() throws IOException {
        // ArrayList<Srna> sRNAs = Srna.getSrnas10403S();
        // TabDelimitedTableReader.save(summarizeResultsInTable(sRNAs),
        // Database.getTEMP_PATH()+"sRNAConservation10403S.txt");
    }
    ...
    public static String[][] summarizeResultsInTable(ArrayList<Srna> sRNAs) {
        ...
        // create a map linking accession ID (found in the Blast result) and the name of
        // the genome (found in my Genome folder)
        ...
        for (int i = 0; i < arrayGenome.length; i++)
            accessionToName.put(arrayGenome[i][2], arrayGenome[i][0]);
        TreeMap<String, Integer> accessionToID = new TreeMap<String, Integer>();
        for (int i = 0; i < arrayGenome.length; i++)
            accessionToID.put(arrayGenome[i][2], Integer.parseInt(arrayGenome[i][1]));
        ...
        for (int i = 0; i < sRNAs.size(); i++) {
            ...
            // read blast result
            String[][] bltResults =
                    TabDelimitedTableReader.read(Database.getTEMP_PATH() + sRNA.getName().replaceAll("/", "") + ".txt");
            ...
            // FileUtils.saveText(phy.toPhyloXML(0),WebUtils.WEBPATH+"sRNAs"+File.separator+"PhyloXML"+File.separator+"PhyloXML_"+sRNA.getName().replaceAll("/",
            // "")+".xml");
            //
            // // create phyloXML table with bacteria only present in the alignment
            ...
            // PhylogenyToolsJolley.addInfoTOGenomePhylogeny(phy);
            // FileUtils.saveText(phy.toPhyloXML(0),WebUtils.WEBPATH+"sRNAs"+File.separator+"PhyloXML"+File.separator+"PhyloXMLAll_"+sRNA.getName().replaceAll("/",
            // "")+".xml");
        }
        return table;
    }
     ...
     * 
     * This method has to be run after <code>multiSpeciesAlign()</code> because it will uses the Blast
     * Result files in Temp folder
     * 
     * @param genome
     */
    public static void addToGenome(String genomeName) {
        ...
        for (Srna sRNA : sRNAs) {
            ...
            String pathSequences = Database.getGENOMES_PATH() + File.separator + genomeName + File.separator
    ...
    public static void createConservationTables() throws Exception {
        ...
        // Phylogeny phy = PhylogenyToolsJolley.readAllBacteriaPhylogeny();
        // String[][] arrayGenome =
        // TabDelimitedTableReader.read(Database.getTEMP_PATH()+"GenomesArray.txt");
    ...
    public static void createConservationTables10403S() throws Exception {
        // ExpressionMatrix allSrna =
        // ExpressionMatrix.readFromFile(Database.getTEMP_PATH()+"sRNAConservation10403S.txt",true);
