...
public class RASToverlap {
	...
	public static String PATH_Conservation = Database.getANALYSIS_PATH()+"Egd-c annotation/Homolog/EGDc_Conservation.txt";
	public static String PATH_Conservation_Cutoff = Database.getANALYSIS_PATH()+"Egd-c annotation/Homolog/EGDc_Conservation_cutoff.txt";
	
	public static void getOverlap(String nCBIGenome,String rASTGenome, String summaryFileName){
		ArrayList<String> up100 = TabDelimitedTableReader.readList(PATH+"comparison/"+summaryFileName+"-sameStopDiffStartPlus100.txt");
		ArrayList<String> noMatch = TabDelimitedTableReader.readList(PATH+"comparison/"+summaryFileName+"-noMatch.txt");
		//	ArrayList<String> diffStop = TabDelimitedTableReader.readList(RASTannotation.PATH+"comparison/"+summaryFileName+"-diffStopDiffStart.txt");
		...
		//	getOverlapList(diffStop, nCBIGenome, rASTGenome, summaryFileName+"-diffStopdiffStart");
	 ...
	 */
	public static void getOverlapList(ArrayList<String> genes, String nCBIGenome,String rASTGenome, String summaryFileName){
		// load both genomes
		...
		for(String geneName : genes){
			...
			if(gene==null) gene = genomeNCBI.getGeneFromName(geneName);
			if(gene!=null){
				System.out.println(gene.getAccession());
				String equivalent = equivalent(gene.getAccession().toString(),false);
				ret += gene.getAccession()+"-"+gene.getBioBegin()+"-"+gene.getBioEnd()+"-"+gene.getLength()+"\t"+equivalent+"\t"+GeneNCBITools.getProduct(gene)+"\t"+GeneNCBITools.searchElement(gene, "Fig family : ")+"\t"+GeneNCBITools.searchElement(gene, "Note : ")+"\t";

				for(DNASequence gElement : rast){
					if(GeneNCBITools.isOverlap(gene, gElement) && !gElement.getAccession().toString().equals(geneName)){
						ret += gElement.getAccession()+"-"+gElement.getBioBegin()+"-"+gElement.getBioEnd()+"-over-"+GeneNCBITools.overlap(gene, gElement);
						ret+="\t";
					}
				}
	...
	/**
	 * Given a sequence name, it gives the conservation in the three different species, in a String[]:<br>
	 *  
	...
	public static String equivalent(String seqName, boolean cutoff){
		...
		if(cutoff) conservationArray = TabDelimitedTableReader.read(PATH_Conservation_Cutoff);
		else conservationArray = TabDelimitedTableReader.read(PATH_Conservation);
			...
			for(int j=0;j<conservationArray[0].length;j++){
				if(seqName.equals(conservationArray[i][j])){
					...
					for(String temp : ArrayUtils.getRow(conservationArray, i)){
						if(!temp.equals(seqName)) ret+=temp+",";
					}
					return ret;
				}
			}
	...
	public static ArrayList<String> getSameEndList(String genome, boolean onlyRAST){
		ArrayList<String> allElement = TabDelimitedTableReader.readList(PATH+"comparison/List "+genome+" sameEnd.txt");
		if(onlyRAST){
			ArrayList<String> finalList = new ArrayList<String>();
			for(String element : allElement){
				if(element.contains("fig")) finalList.add(element);
			}
			return finalList;
	...
	public static ArrayList<String> getNotAssociatedList(String genome, boolean onlyRAST){
		ArrayList<String> allElement = TabDelimitedTableReader.readList(PATH+"comparison/List "+genome+" notAssoc.txt");
		if(onlyRAST){
			ArrayList<String> finalList = new ArrayList<String>();
			for(String element : allElement){
				if(element.contains("fig")) finalList.add(element);
	...
	public static void getEGDeAutoOverlap(){
		...
		results.add(ret);
		ArrayList<String> finalList = TabDelimitedTableReader.readList(PATH+"comparison/"+"EGD-e genes with diff start and overlap.txt");
		int k=0;
		for(DNASequence seq : genome.getCodingSequencesList(true)){
			for(DNASequence seq2 : genome.getCodingSequencesList(true)){
				if(!seq.equals(seq2) && GeneNCBITools.isOverlap(seq, seq2)){
					if(k==1){
						k=0;  // if k==1 it means last step an overlap was found so we need to skip this step otherwise we will have a doublon
					}else{
						String found = "";
						if(finalList.contains(seq.getAccession().toString())) found = seq.getAccession().toString();
						if(finalList.contains(seq2.getAccession().toString())) found = seq2.getAccession().toString();
						results.add(seq.getAccession().toString()+"\t"+seq2.getAccession().toString()+"\t"+GeneNCBITools.overlap(seq,seq2)+"\t"+found);
						k=1;
					}
				}
			}
