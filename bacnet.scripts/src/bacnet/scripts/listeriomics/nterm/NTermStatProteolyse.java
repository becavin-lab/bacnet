...
public class NTermStatProteolyse {
    ...
    // }
    // //porte306
    // System.out.println("NB of groups removed because it corresponds to same peptide but with
    // different type of modification: "+multimodif);
    ...
    public static void saveProteolyses(ArrayList<TIS> tiss, String fileName) {
        ...
        String header =
                "TIS name\tRef sequence\tList of modification\tList of overlaps\tRef peptide\t# peptides\t# spectra";
        results.add(header);
        ...
        for (TIS tisnTerm : tiss) {
            ...
            // row =
            // tisnTerm.getName()+"\t"+nTermTemp.getSequencePeptide()+"\t"+nTermTemp.getTypeModif()+"\t"+nTermTemp.getOverlap()+"\t"+nTermTemp.getSpectra()+"\t"+nTermTemp.getTypeOverlap()+"\t"+nTermTemp.getName();
            // results.add(row);
            // }
        }
