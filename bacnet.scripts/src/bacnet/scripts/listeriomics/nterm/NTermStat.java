...
//		setTIRSequence();
	...
	public void display(){
		ExpressionMatrix matrix = ExpressionMatrix.loadTab(NTermUtils.getPATH()+"Statistics/Statistics Genes.excel",true);
...
//		TableAWTView.displayMatrix(matrix, "heatMap");
	...
	public ArrayList<Gene> getiTIS(){
		...
		for(int i=1;i<arrayiTIS.length;i++){
			...
			geneITIS.getFeatures().put("Formyl", arrayiTIS[i][ArrayUtils.findColumn(arrayiTIS, "For or non-AUG start?")]);
	 ...
	 * 
	 * The Translation Iinitiation region here is always:
	 * -20bp ----  60bp around start codon
	 * <li> Start codon
	 * <li> SD energy
	 * <li> Structure energy
	 * ....
	...
	public void tableGeneEGDe(){
		...
		String[] headers = {"Name","Length (aa)","Start codon","SD energy","TIR structure (-20 to +60bp)","A-T number (TIR)","G-C number(TIR)","GC number","Codon adaptation Index of TIR",
				"Codon adaptation Index of Gene","Charged residues proportion (TIR)","Charged residues proportion (Gene)",
				"GeneExpr: BHI 37C Exp 1","GeneExpr: BHI 37C Exp 2","GeneExpr: BHI 37C Stat","GeneExpr: BHI 30C","GeneExpr: Human blood 60min","UTR length","Total nb spectra","N-Term aTIS Exp1","N-Term aTIS Exp2","N-Term aTIS Blood","Detectable ?","ncRNA (-100bp upstream)",
				"Gene","Description","COG","Strand","Begin","End","TIRSequence (-"+DECAY_MINUS+":+"+DECAY_PLUS+" around start codon)"};
		String header = "";
		...
		for(int i = 0;i<listGenes.size();i++){
			...
			if(decay>gene.getLength()) decay = gene.getLength()-1;
			...
			String[] rows = {gene.getName(),gene.getLengthAA()+"",startCodon,sdEnergy,tirStructure,atNumber,gcNumber,gcNumberGene,codonCAI,codonCAIGene,
					chargedResidues,chargedResiduesGene,expTilingBHI1,expTilingBHI2,expTilingStat,expTiling30C,expTilingBlood,utrLength,totalNbSpectrum,aTISExp1,aTISExp2,aTISBlood,detectable,ncRNAUpstream,
					gene.getGeneName(),comment,gene.getCog(),gene.getStrand()+"",gene.getBegin()+"",gene.getEnd()+"",tirSequence};
			String row = "";
	...
	public static double getAUNumber(String sequence){
		return ((double)StringUtils.countMatches(sequence, "A")+(double)StringUtils.countMatches(sequence, "U"))/(double)sequence.length();
	}
	/**
	...
	public static double getGCNumber(String sequence){
		return ((double)StringUtils.countMatches(sequence, "G")+(double)StringUtils.countMatches(sequence, "C"))/(double)sequence.length();
	}
	...
	public static String getComment(Gene gene){
		if(gene.getName().contains("iTIS")){
			String commentTemp = "LengthDiff:"+gene.getFeature("length difference")+";Peptide:"+gene.getFeature("peptide")+";Gene:"+gene.getFeature("Gene")+";"+gene.getComment();
			return commentTemp;
		}else{
	...
	public static double getStartCodon(Gene gene){
		...
		//		String sdEnergy = ""+UNAfold.hybridRNA(startCodon,"start", "AUG","anti-SD",true);
	...
	public static String getSDEnergy(Gene gene){
		String[][] geneArray = TabDelimitedTableReader.read(NTermUtils.getPATH()+"Statistics/StatGenes/aSDBindingEnergy.excel");
	...
	public void setSDBindingEnergy(){
		...
		for(Gene gene : listGenes){
			...
			double sdEnergy = UNAfold.hybridRNA(gene.getSDSequence(),gene.getName(), Sequence.ANTI_SD_SEQ,"anti-SD"+k,true);
			row+=sdEnergy;
			result.add(row);
			k++;
		}
	...
	public static String getTIRStructureEnergy(Gene gene){
		String[][] geneArray = TabDelimitedTableReader.read(NTermUtils.getPATH()+"Statistics/StatGenes/TIRStructure.excel");
		for(int i=0;i<geneArray.length;i++){
			if(geneArray[i][0].equals(gene.getName())){
				return geneArray[i][1]+"";
			}
	...
	/**
	 * For every gene calculate the 2nd structure free energy and save it a table<br>
	...
	public void setTIRStructureEnergy(){
		...
		for(Gene gene : listGenes){
			...
			for(int i=0;i<40 && ((i+(DECAY_PLUS-20))<gene.getLength());i++){
				String tirSequence = gene.getSDSequence(DECAY_MINUS) + gene.getSequenceRNA().substring(3, (DECAY_PLUS-20)+i);
			...
			k++;
			if(k%500==0){
				TabDelimitedTableReader.saveList(result, NTermUtils.getPATH()+"Statistics/StatGenes/TIRStructure_"+k+".excel");
			}
		}
	...
	/**
	 * After generating TIRSequence.fasta in setTIRSequence() method we submit to http://genomes.urv.cat/CAIcal/
	 * to calculate CAI value and expected CAI value 
	 * The table with CAI values is saved from the website in: CAIResults.excel
	 * @param gene
	 * @return
	 */
	public String getCAI(Gene gene){
		String[][] geneArray = TabDelimitedTableReader.read(NTermUtils.getPATH()+"Statistics/StatGenes/CAIResults.excel");
		for(int i=0;i<geneArray.length;i++){
			if(geneArray[i][0].equals(gene.getName())){
				return geneArray[i][1]+"";
			}
	...
	/**
	 * After generating GeneSequence.fasta in setGeneSequence() method we submit to http://genomes.urv.cat/CAIcal/ to calculate CAI value and expected CAI value
	 * The table with CAI values is saved from the website in: StatGenes/CAIGeneResults.excel
	 * @param gene
	 * @return
	 */
	public String getCAIGene(Gene gene){
		String[][] geneArray = TabDelimitedTableReader.read(NTermUtils.getPATH()+"Statistics/StatGenes/CAIGeneResults.excel");
		for(int i=0;i<geneArray.length;i++){
			if(geneArray[i][0].equals(gene.getName())){
				return geneArray[i][1]+"";
			}
	...
	/**
	 * By reading the List in:
	 * /StatGenes/EGDeDetectable.txt
	 * Say if a gene is detectable by the spectometer or not
	 * 
	 * @param gene
	 * @return
	 */
	public String getDetectable(Gene gene){
		ArrayList<String> genes = TabDelimitedTableReader.readList(NTermUtils.getPATH()+"Statistics/StatGenes/EGDeDetectable.txt");
...
//					}
	...
	/**
	 * For each gene search the closest TSS downstream, and return the UTR length (=distance between gene and TSS)
	 * We don't start at the start codon but -20bp, in case there is a TSS in this region indicating a possible short UTR with alternative aTIS
	 * @param gene
	 * @return
	 */
	@Deprecated
...
//		}
	...
	public double getGeneExpr(Gene gene,String dataName){
		...
		if(name.contains("iTIS")) name = name.replaceFirst("iTIS ","").split("_")[0];
	...
	public int getTotalSpectrum(Gene gene){
		...
		int count = 0;
		String name = gene.getName();
		if(name.contains("iTIS")) name = gene.getFeature("Gene");
		for(TIS tis : massSpec.getTisList()){
	...
	public static void detectable(){
		 ...
		 */
		String[][] predictedPeptide = TabDelimitedTableReader.read(NTermUtils.getPATH()+"EGDePeptideMass_"+cutPeptide+".txt");
		ArrayList<String> predicted = new ArrayList<String>();
		ArrayList<String> notPredicted = new ArrayList<String>();
		String[] headers = {"Sequence","Start codon","Locus","Theoretical pI","Molecular weight","Gene","Description","COG","Strand","Begin","End"};
		String header = "";
		for(String temp : headers) header +=temp+"\t";
			...
			if(sequence.length()>5){
				...
				for(int i=1;i<predictedPeptide.length;i++){
					...
					if(sequence.equals(seq)){
						...
						String[] rows = {seq,gene.getSequence().substring(0, 3),gene.getName(),pI+"",mass+"",gene.getGeneName(),gene.getComment(),gene.getCog(),gene.getStrand()+"",gene.getBegin()+"",gene.getEnd()+""};
						String row = "";
						for(String temp : rows) row +=temp+"\t";
				...
				if(!found) System.err.println("not found"+ gene.getName());
			}
			else{
				String[] rows = {sequence,gene.getSequence().substring(0, 3),gene.getName(),"","",gene.getGeneName(),gene.getComment(),gene.getCog(),gene.getStrand()+"",gene.getBegin()+"",gene.getEnd()+""};
				String row = "";
				for(String temp : rows) row +=temp+"\t";
				notPredicted.add(row);
			}
	 ...
	 * <li> number of predicted genes<br><br>
	...
	public static void statInternalORF(Genome genome){
		...
		for(int i=2;i<genome.getFirstChromosome().getLength();i++){
			...
			else if(Codon.isStop(codon)) numberStop++;
		...
		for(Gene gene : genome.getFirstChromosome().getGenes().values()){
			...
			geneSDsequences.add(gene.getName()+"\t"+numberInternalSD+"\t"+gene.getStrand()+";"+gene.getBegin()+";"+gene.getEnd()+"\t"+gene.getProduct()+"\t"+gene.getGeneName());
		}
		TabDelimitedTableReader.saveList(geneSDsequences, NTermUtils.getPATH()+"Internal_SD_stat-"+genome.getSpecies()+".txt");
	...
	public static void peptideLogo(){
		 ...
		 * Logo of aTIS
		 */
		//		ArrayList<String> lmosaTIS = TabDelimitedTableReader.readList(NTermUtils.getPATH()+"List peptides/aTIS List.txt");
		//		for(String aTISlmo : lmosaTIS){
		//			Sequence gene = genome.getElement(aTISlmo);
		//			fastaFile.add(">"+aTISlmo);
		//			fastaFile.add(getLogo(gene));
		//		}
		//		TabDelimitedTableReader.saveList(fastaFile, NTermUtils.getPATH()+"List peptides/aTIS logo.fasta");
		/*
		 * Logo of all Lmos
		 */
		fastaFile.clear();
		...
		//		 */
		//		ArrayList<String> highSDlmo = TabDelimitedTableReader.readList(NTermUtils.getPATH()+"List peptides/highSD lmo.txt");
		//		for(String aTISlmo : highSDlmo){
		//			Sequence gene = genome.getElement(aTISlmo);
		//			fastaFile.add(">"+aTISlmo);
		//			fastaFile.add(getLogo(gene));
		//		}
		//		TabDelimitedTableReader.saveList(fastaFile, NTermUtils.getPATH()+"List peptides/highSD logo.fasta");
		//		
		//		
		//		TabDelimitedTableReader.saveTreeSet(noATISfound, NTermUtils.getPATH()+"List peptides/no aTIS lmo List.txt");
		//		/*
		//		 * Logo of lmos for which no aTIS exists
		//		 */
		//		fastaFile.clear();
		//		ArrayList<String> nolmosaTIS = TabDelimitedTableReader.readList(NTermUtils.getPATH()+"List peptides/no aTIS lmo List.txt");
		//		for(String aTISlmo : nolmosaTIS){
		//			Sequence gene = genome.getElement(aTISlmo);
		//			fastaFile.add(">"+aTISlmo);
		//			fastaFile.add(getLogo(gene));
		//		}
		//		TabDelimitedTableReader.saveList(fastaFile, NTermUtils.getPATH()+"List peptides/no aTIS logo.fasta");

		//		/*
		//		 * Logo of all iTIS
		//		 */
		//		String[][] iTIS = TabDelimitedTableReader.read(NTermUtils.getPATH()+"List peptides/iTIS List.txt");
		//		fastaFile.clear();
		...
		//			fastaFile.add(getLogo(gene));
		//		}
		//		TabDelimitedTableReader.saveList(fastaFile, NTermUtils.getPATH()+"List peptides/iTIS logo.fasta");
