...
/**
 * List of method to do statistics about common features in N-Terminus of genes: - start codon - SD
 * binding - AU proportion - Folding energy
 * 
 * @author UIBC
 *
 */
public class NTermStat {
     ...
     * 
     * The Translation Iinitiation region here is always: -20bp ---- 60bp around start codon
     * <li>Start codon
     * <li>SD energy
     * <li>Structure energy ....
     */
    ...
    public static String getSDEnergy(Gene gene) {
        String[][] geneArray =
                TabDelimitedTableReader.read(NTermUtils.getPATH() + "Statistics/StatGenes/aSDBindingEnergy.excel");
    ...
    public void setSDBindingEnergy() {
        ...
        for (Gene gene : listGenes) {
            ...
            double sdEnergy =
                    UNAfold.hybridRNA(gene.getSDSequence(), gene.getName(), Sequence.ANTI_SD_SEQ, "anti-SD" + k, true);
            row += sdEnergy;
            result.add(row);
            k++;
        }
    ...
    /**
     * For very gene read TIRStructure.excel and extract the mean value of energy: We calculate the 2nd
     * structure free energy and save it a table<br>
     * And we calculate the energy for 40 different sequence: -20+40 to -20+80 and calculate the average
     * energy
     * 
     * @param gene
     * @return
    ...
    public static String getTIRStructureEnergy(Gene gene) {
        String[][] geneArray =
                TabDelimitedTableReader.read(NTermUtils.getPATH() + "Statistics/StatGenes/TIRStructure.excel");
        for (int i = 0; i < geneArray.length; i++) {
            if (geneArray[i][0].equals(gene.getName())) {
                return geneArray[i][1] + "";
            }
     ...
     * We calculate the energy for 40 different sequence: -20+40 to -20+80 and calculate the average
     * energy
     */
    public void setTIRStructureEnergy() {
        ...
        for (Gene gene : listGenes) {
            ...
            for (int i = 0; i < 40 && ((i + (DECAY_PLUS - 20)) < gene.getLength()); i++) {
                String tirSequence =
                        gene.getSDSequence(DECAY_MINUS) + gene.getSequenceRNA().substring(3, (DECAY_PLUS - 20) + i);
     ...
     * http://genomes.urv.cat/CAIcal/ to calculate CAI value and expected CAI value The table with CAI
     * values is saved from the website in: CAIResults.excel
     * 
     * @param gene
     * @return
     */
    public String getCAI(Gene gene) {
        String[][] geneArray =
                TabDelimitedTableReader.read(NTermUtils.getPATH() + "Statistics/StatGenes/CAIResults.excel");
        for (int i = 0; i < geneArray.length; i++) {
            if (geneArray[i][0].equals(gene.getName())) {
                return geneArray[i][1] + "";
            }
     ...
     * http://genomes.urv.cat/CAIcal/ to calculate CAI value and expected CAI value The table with CAI
     * values is saved from the website in: StatGenes/CAIGeneResults.excel
     * 
     * @param gene
     * @return
     */
    public String getCAIGene(Gene gene) {
        String[][] geneArray =
                TabDelimitedTableReader.read(NTermUtils.getPATH() + "Statistics/StatGenes/CAIGeneResults.excel");
        for (int i = 0; i < geneArray.length; i++) {
            if (geneArray[i][0].equals(gene.getName())) {
                return geneArray[i][1] + "";
            }
    ...
    /**
     * By reading the List in: /StatGenes/EGDeDetectable.txt Say if a gene is detectable by the
     * spectometer or not
     * 
     * @param gene
     * @return
    ...
    public String getDetectable(Gene gene) {
        ArrayList<String> genes =
                TabDelimitedTableReader.readList(NTermUtils.getPATH() + "Statistics/StatGenes/EGDeDetectable.txt");
    ...
    /**
     * For each gene search the closest TSS downstream, and return the UTR length (=distance between
     * gene and TSS) We don't start at the start codon but -20bp, in case there is a TSS in this region
     * indicating a possible short UTR with alternative aTIS
     * 
     * @param gene
     * @return
     */
    ...
    /**
     * Return the median value expression (calculated on 10 BHI 37C data) in the Gene Expression array
     * of the gene
     * 
     * @param gene
     * @return
    ...
    /**
     * For each gene search if the end of a smallRNA or cisReg element is in the region -200bp - start
     * codon
     * 
     * @param gene
     * @return
    ...
    public static void detectable() {
         ...
         */
        String[][] predictedPeptide =
                TabDelimitedTableReader.read(NTermUtils.getPATH() + "EGDePeptideMass_" + cutPeptide + ".txt");
    ...
    public static void peptideLogo() {
        ...
        ArrayList<String> lmosaTIS =
                TabDelimitedTableReader.readList(NTermUtils.getPATH() + "List peptides/aTISList.txt");
