...
public class NTermTranscriptome {
	...
	public static void getTranscriptomes(){
		...
		ExpressionMatrix matrix = ExpressionMatrix.load(Database.getLOGFC_MATRIX_TRANSCRIPTOMES_PATH()+"_"+Genome.EGDE_NAME+"_PRIVATE");
	...
	public static void getRegulatoryRegion(){
		ArrayList<String> listSeqNames = TabDelimitedTableReader.readList(Database.getInstance().getPath()+"H2O2Transcriptomes_PosReg.txt");
		...
		TabDelimitedTableReader.saveList(finalResult, Database.getInstance().getPath()+"H2O2Transcriptomes_PosReg.fasta");
	}
}
