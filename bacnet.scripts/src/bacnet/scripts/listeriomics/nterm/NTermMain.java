...
import bacnet.datamodel.proteomics.NTermFilter;
import bacnet.datamodel.proteomics.NTermSummaryTable;
...
import bacnet.scripts.phylogeny.PhylogenyRli42vsRsbR;
...
//		System.out.println(Genome.loadEgdeGenome().getFirstChromosome().getSequenceAsString(204353, 204624, Strand.NEGATIVE));
	...
	public static void createListeriomicsTable(String fileName){
		...
		String[] headers = {"PeptideID","Peptide Type","Type Modif.","Peptide Sequence","Peptide Begin","Peptide End","Strand","Nb. Spectra","Threshold","Score",
				"Protein Overlap","DistanceOverlap","Size Protein (aa)","Protein Begin","Protein End","Sequence Protein"};
		String header ="";
		...
		for(String nTermID : ntermData.getElements().keySet()){
			...
			nTerm.getSequencePeptide()+"\t"+nTerm.getBegin()+"\t"+nTerm.getEnd()+"\t"+nTerm.getStrand();
				...
				if(overlaps.length>1){
					...
					if(geneOverlap.contains("rli")){
						...
						geneInfo = sRNA.getName()+"\t"+overlaps[1]+"\t"+sRNA.getSequenceAA().length()+"\t"+sRNA.getBegin()+"\t"+sRNA.getEnd()+"\t"+sRNA.getSequenceAA();
					}else if(geneOverlap.contains("lmo")){
						Gene gene = chromo.getGenes().get(geneOverlap);
						System.out.println(geneOverlap+" - ");
						geneInfo = gene.getName()+"\t"+overlaps[1]+"\t"+gene.getSequenceAA().length()+"\t"+gene.getBegin()+"\t"+gene.getEnd()+"\t"+gene.getSequenceAA();
					}
					System.out.println(geneOverlap);
				}
	...
	public static void findLeaderLess(){
		String[][] array = TabDelimitedTableReader.read(NTermUtils.getPATH()+"Statistics/LeaderLess_EGDe_protein.txt");
		ArrayList<String> aTISGenes = TabDelimitedTableReader.readList(NTermCreateFinalData.PATH_LIST+"aTIS_Francis_19052015.txt");
		for(int i=1;i<array.length;i++){
