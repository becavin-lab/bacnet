...
public class NTermDatabase {
     ...
     * <li>all ASrna on 3 frames (300bp long) lists of Genes, ID=lmo, directly taken from NCBI lists of
     * "new"Gene, ID=rastlmo, from the analysis of annotation I just did using RAST software lists of
     * Srna, ID=rli, rliM, rliP, we don't know where is the begining so we have 3 frame for each rli, we
     * start at the first start codon, plus we cut when we found stop codon and peptide smaller than
     * 5aa. lists of ASrna, ID=anti, antiM, antiP, we don't know where is the begining so we have 3
     * frame for each asRNA, we start at the first start codon, plus we cut when we found stop codon and
     * peptide smaller than 5aa.
     * 
     */
    public static void createProteinDB() {
        ...
        for (Gene gene : genome.getFirstChromosome().getGenes().values()) {
            ...
            if (name.equals("- "))
                name = "";
            ret += ">protDB|" + gene.getName() + "|EGDe " + name + gene.getBegin() + ".." + gene.getEnd() + "/"
                    + gene.getStrand() + "\n";
        ...
        for (Srna sRNA : genome.getFirstChromosome().getsRNAs().values()) {
            ...
            for (String sequence : sequences) {
                if (sequence.length() > LENGTH_CUTOFF) {
                    ret += ">protDB|" + name + "_" + i + "|EGDe " + sRNA.getBegin() + ".." + sRNA.getEnd() + "/"
                            + sRNA.getStrand() + "\n";
                    ret += sequence + "\n";
                }
                i++;
            ...
            for (String sequence : sequences) {
                if (sequence.length() > LENGTH_CUTOFF) {
                    ret += ">protDB|" + name + "M_" + i + "|EGDe " + seqM.getBegin() + ".." + seqM.getEnd() + "/"
                            + seqM.getStrand() + "\n";
                    ret += sequence + "\n";
                }
                i++;
            ...
            for (String sequence : sequences) {
                if (sequence.length() > LENGTH_CUTOFF) {
                    ret += ">protDB|" + name + "P_" + i + "|EGDe " + seqP.getBegin() + ".." + seqP.getEnd() + "/"
                            + seqP.getStrand() + "\n";
                    ret += sequence + "\n";
                }
                i++;
        ...
        for (Srna sRNA : genome.getFirstChromosome().getAsRNAs().values()) {
            ...
            for (String sequence : sequences) {
                if (sequence.length() > LENGTH_CUTOFF) {
                    ret += ">protDB|" + sRNA.getName() + "_" + i + "|EGDe " + sRNA.getBegin() + ".." + sRNA.getEnd()
                            + "/" + sRNA.getStrand() + "\n";
                    ret += sequence + "\n";
                }
                i++;
            ...
            for (String sequence : sequences) {
                if (sequence.length() > LENGTH_CUTOFF) {
                    ret += ">protDB|" + sRNA.getName() + "M_" + i + "|EGDe " + seqM.getBegin() + ".." + seqM.getEnd()
                            + "/" + seqM.getStrand() + "\n";
                    ret += sequence + "\n";
                }
                i++;
            ...
            for (String sequence : sequences) {
                if (sequence.length() > LENGTH_CUTOFF) {
                    ret += ">protDB|" + sRNA.getName() + "P_" + i + "|EGDe " + seqP.getBegin() + ".." + seqP.getEnd()
                            + "/" + seqP.getStrand() + "\n";
                    ret += sequence + "\n";
                }
                i++;
     ...
     * We found the correct frame so we are sure to have the start codon of the gene at the exact
     * position of NCBI annotation We start at the first start codon, then if stop codon are found in
     * the UTR region we cut in different peptides, remove ones < 5aa, and paste to the last part of the
     * UTR the whole gene amino acide sequence all gene, ID=lmo_n n being the id of the UTR peptide
     * extracted
     */
    public static void createUTRDB() {
        ...
        for (int i = 1; i < tssTable.length; i++) {
            ...
            int k = 0;
            int count = 1;
            // sequencesFinal.add(">utrDB|"+name+"_"+k+"|"+seq.getBegin()+".."+seq.getEnd()
            // +" utrLength "+utrLength); // to test if we cut accordingly the UTR
            // sequencesFinal.add(seq.getSequenceAA());
            for (String sequence : sequences) {
                if (count == sequences.size()) {
                    sequencesFinal.add(">utrDB|" + name + "_" + k + "|EGDe " + seq.getBegin() + ".." + seq.getEnd()
                            + "/" + seq.getStrand() + " utrLength " + utrLength);
                    k++;
     ...
     * The small peptides include in the list are the one only of size more than 5aa, and surrounded by
     * at least one R.<br>
     ...
     * <li>frame 6 = EGDe reverse complement genome 1bp on the right, ID=genomeComplP <br>
     */
    public static void createGenomeDB() {
        ...
        createGenomeFasta(chromoComplement.getRNASequence().getProteinSequence().getSequenceAsString(), "genomeCompl",
                "EGDeCompl");
        ...
        createGenomeFasta(seq.getRNASequence().getProteinSequence().getSequenceAsString(), "genomeComplM",
                "EGDeComplMinus");
        seq = new DNASequence(seqPlus);
        createGenomeFasta(seq.getRNASequence().getProteinSequence().getSequenceAsString(), "genomeComplP",
                "EGDeComplPlus");

        ArrayList<String> ret;
        ret = TabDelimitedTableReader.readList(DATABASE + "GenomeDB_EGDe.fasta");
        for (String line : TabDelimitedTableReader.readList(DATABASE + "GenomeDB_EGDeCompl.fasta"))
            ret.add(line);
        for (String line : TabDelimitedTableReader.readList(DATABASE + "GenomeDB_EGDeMinus.fasta"))
            ret.add(line);
        for (String line : TabDelimitedTableReader.readList(DATABASE + "GenomeDB_EGDePlus.fasta"))
            ret.add(line);
        for (String line : TabDelimitedTableReader.readList(DATABASE + "GenomeDB_EGDeComplMinus.fasta"))
            ret.add(line);
        for (String line : TabDelimitedTableReader.readList(DATABASE + "GenomeDB_EGDeComplPlus.fasta"))
            ret.add(line);
        TabDelimitedTableReader.saveList(ret, DATABASE + "GenomeDB.fasta");
    }

    /**
     * From an amino acid sequence of a genome create final fasta file by cutting at each Start, Stop
     * and R positions
     * 
     * @param seqAA
     * @param prefix
            ...
            if (parses.length == 3) {
                ...
                if (begin.equals("R") || end.equals("R")) {
                    ...
                    if (seq.length() > 4) {
                        ...
                        sequences.add(">genoDB|" + prefix + "_" + k + "|EGDe " + (pos * 3) + ".."
                                + ((pos + seq.length()) * 3));
                        sequences.add(seq);
    ...
    /**
     * Create database of all peptide from a start codon (M=ATG, L=TTG, V=GTG), to the next stop codon
     * or R
     */
    public static void createTISDB() {
        ...
        for (int i = 0; i < Codon.startCodon.length; i++) {
            ...
            getAllPeptides(sequence, startOccurences, peptides, Codon.startCodon[i][1].charAt(0),
                    Codon.startCodon[i][0], 2944527, true, 10000);
        }

        String seqComplement = chromosome.getReverseComplement().getSequenceAsString();
        for (int i = 0; i < Codon.startCodon.length; i++) {
            startOccurences = FileUtils.searchPosition(Codon.startCodon[i][0], seqComplement);
            getAllPeptides(seqComplement, startOccurences, peptides, Codon.startCodon[i][1].charAt(0),
                    Codon.startCodon[i][0], 2944527, false, 10000);
        }
    ...
    /**
     * Divide TISDB into sequence starting with a M (TISMDB), and other not starting with an M
     * (TISnoMDB).
     */
    ...
    /**
     * Given a genome sequence, and a list of position of start codon, update a list of peptides
     * accordingly<br>
    ...
    public static void getAllPeptides(String sequence, ArrayList<Integer> startOccurences, ArrayList<String> peptides,
        ...
        for (Integer position : startOccurences) {
            ...
            if (index == -1 && index == -1) {
                ...
                if (subSeqAA.length() > (LENGTH_CUTOFF - 1)) {
                    if (subSeqAA.length() < cutoff) {
                        ...
                        /*
                         * If we are on minus strand working with reverse complement sequence the real position on the
                         * genome is : genomeLength-position
                         */
            ...
            // System.out.println(subSeqAATemp+" keep: "+subSeqAA); // verify that we really
            // cut before stop codon
    ...
    public static void verifyTISDB() {
        ...
        /*
         * Check if every peptide has either "no R" or "an R at the end", because no R in the middle of the
         * peptide is allowed
         */
        ...
        for (Gene gene : genome.getFirstChromosome().getGenes().values()) {
            ...
            if (seqAA.indexOf('R') != -1)
                seqAA = seqAA.substring(0, seqAA.indexOf('R') + 1);
            if (seqAA.length() > 4) {
    ...
    public static void createMDB() {
        ...
        String seqComplement = chromosome.getReverseComplement().getSequenceAsString();
        DNASequence chromoComplement = new DNASequence(seqComplement);
        createMDBFasta(chromoComplement.getRNASequence().getProteinSequence().getSequenceAsString(), "genomeCompl",
                "EGDeCompl");
        ...
        seqPlus = seqTemp.substring(1, seqTemp.length()) + seqTemp.charAt(0);
        seq = new DNASequence(seqMinus);
        createMDBFasta(seq.getRNASequence().getProteinSequence().getSequenceAsString(), "genomeComplM",
                "EGDeComplMinus");
        seq = new DNASequence(seqPlus);
        createMDBFasta(seq.getRNASequence().getProteinSequence().getSequenceAsString(), "genomeComplP",
                "EGDeComplPlus");

        ArrayList<String> ret;
        ret = TabDelimitedTableReader.readList(DATABASE + "MDB_EGDe.fasta");
        for (String line : TabDelimitedTableReader.readList(DATABASE + "MDB_EGDeCompl.fasta"))
            ret.add(line);
        for (String line : TabDelimitedTableReader.readList(DATABASE + "MDB_EGDeMinus.fasta"))
            ret.add(line);
        for (String line : TabDelimitedTableReader.readList(DATABASE + "MDB_EGDePlus.fasta"))
            ret.add(line);
        for (String line : TabDelimitedTableReader.readList(DATABASE + "MDB_EGDeComplMinus.fasta"))
            ret.add(line);
        for (String line : TabDelimitedTableReader.readList(DATABASE + "MDB_EGDeComplPlus.fasta"))
            ret.add(line);
    ...
    public static void possibleSmallRNAPeptides() {
        // String[][] array = TabDelimitedTableReader.read(NTermUtils.getPATH()+"Peptide candidates - sRNAs
        // - 20-09-11.txt");
        // String cutPeptide = "E";
        String cutPeptide = "R";
         ...
         */
        // String[][] array = TabDelimitedTableReader.read(NTermUtils.getPATH()+"GluC
        // sRNA.txt");
