...
public class NTermCreateFinalData {
	...
	/**
	 * From the list created by Francis, update all Nterm peptides, indicating which type of TIS (translation initiation sites) they are:
	 * annotated TIS
	 * multiple TIS
	 * updated TIS
	 * internal TIS
	...
	/**
	 * Load list of gene having an updated TIS peptide and search for every NTerm which are exactly at the beginning of the gene or +3 nucleotide after
	 */
	private void getNewTIS(){
		...
		for(int j=0;j<multipleTISGenes.length;j++){
			...
			/*
			 * For alternative TIS position get NTerms and update their overlap value and create a TIS for them
			 */
			...
			for(NTerm nTerm : allNtermTemp){
				if(nTerm.isStrand()){
					if(nTerm.getBegin() == startalternateNTerm){
						System.out.println(nTerm.getName()+" "+startalternateNTerm+" "+nTerm.getSequencePeptide());
	...
	/**
	 * Load list of gene having a multiple TIS peptide and search for every NTerm which are exactly at the beginning of the gene or +3 nucleotide after
	 */
	private void getMultipleTIS(){
		...
		for(int j=0;j<multipleTISGenes.length;j++){
			...
			for(TIS tis : massSpecDataNew.getTisList()){
				if(tis.getOverlapingGene().equals(gene) && tis.getOverlapsString().contains(NTermData.TYPE_OVERLAPS[0])){
	...
	/**
	 * Load list of gene having an aTIS peptide and search for every NTerm which are exactly at the beginning of the gene or +3 nucleotide after
	 */
	private void getATIS(){
		...
		for(String lmo : aTISGenes){
			...
			/*
			 * Create NTerms
			 * //System.out.println("Search all NTerms for : "+lmo);
			 */
			if(gene.isStrand()){
	...
	private void verifyTIS(){
		System.out.println("Classify "+massSpecDataNew.getElements().size()+" NTerms in TIS - we have "+massSpecDataNew.getTisMap().size()+" classified");
	...
	private void getUncategorized(){
		...
		for(NTerm nTerm : allNterms){
			if(nTerm.getTypeOverlap().equals("uncategorized")){
				...
				System.out.println("cannot classified: "+nTerm.getName()+" : "+nTerm.getOverlap()+": "+nTerm.getTypeOverlap());
			}
		}
		System.out.println(count+" : peptides uncategorized remaining (for a total of: "+massSpecDataNew.getNTerms().size()+" peptides)");
	}
	...
	private void createATISTable(){
		...
		for(TIS tis : massSpecDataNew.getTisList()){
			...
			if(tis.getnTerms().get(0).getTypeOverlap().equals("aTIS")){
				...
				if(tis.getnTermRef().getTypeModif()==TypeModif.For) seqPeptide = "For-"+seqPeptide;
				results.add(tis.getnTerms().get(0).getOverlap().split(" :")[0]+"\t"+tis.getName()+"\t"+tis.getFormylatedExperiment()+"\t"+tis.isFormylated()+"\t"+tis.getRefSequence()+"\t"+tis.getnTermsSequences()+
						"\t"+tis.getTotalSpectra()+"\t"+seqPeptide+"\t"+tis.getnTermRef().getScore()+"\t"+tis.getnTermRef().getThreshold()+"\t"+tis.getUTRLength());
			}
		}
