...
public class SystemsBiologyListeriomics {
    ...
    private static void createCircularGenomeView() {
        ...
        try {
            ...
            // String fileTempPNG = FileUtils.removeExtension(Network.CIRCOS_BACK_PATH)+".png";
            System.out.println("file " + fileTempPNG);
            CgviewIO.writeToPNGFile(panel.getCgview(), fileTempPNG);
            // CgviewIO.writeToSVGFile(panel.getCgview(), Network.CIRCOS_BACK_PATH, false, false);
        } catch (IOException e) {
    ...
    private static void createCoExpressionMatrix(Experiment exp, Genome genome) {
        ExpressionMatrix matrixLOGFC =
                ExpressionMatrix.load(Database.getLOGFC_MATRIX_TRANSCRIPTOMES_PATH() + "_" + genome.getSpecies());
