...
public class ComparisonsCreation {
	...
	/**
	 * Go through all comparisons and create for each <code>Tiling</code> and
	 * <code>GenExpression</code> different matrices giving the number of genes,
	 * Srna and asRNA overexpressed and underexpressed
	 * 
	 * @param compList
	 */
	...
	/**
	 * Calculate a comparison, filter the results, and save ExpressionMatrix and
	 * list of elements up-regulated and down-regulated
	 * 
	 * @param elements
	 * @param ID
	 * @param comparison
	...
	public static void calcFilterAndSave(ArrayList<String> elements, String ID, String comparison,
		...
		if (compMatrix != null) {
			...
			// TabDelimitedTableReader.saveList(allExpressed,
			// path+comparison+"_"+ID+"_List.txt");
			// TabDelimitedTableReader.saveList(overExpressed,
			// path+comparison+"_"+ID+"_Over_List.txt");
			// TabDelimitedTableReader.saveList(underExpressed,
			// path+comparison+"_"+ID+"_Under_List.txt");
	...
	/**
	 * Reorder headers of the comparison Matrix to group GenExpression and Tiling
	 * results coming from the same comparison <br>
	 * 
	 * @param compMatrix
	 * @param tests
	 * @param comp
	...
	public static ExpressionMatrix reorganizeMatrix(ExpressionMatrix compMatrix, ArrayList<StatTest> tests,
		...
		for (StatTest test : tests) {
			if (test.getType() == TypeStat.AUTO_BOTH) {
				// first calculate mean of FDRBY and FDRBonf on GeneExpression and Tiling for
				// each comparison
				...
				for (int i = 0; i < comp.getLeftBCs().size(); i++) {
					...
					for (String rowName : compMatrix.getRowNames().keySet()) {
						// FDRBY
						double mean = (compMatrix.getValue(rowName,
								"STAT_" + name + "_FDRBY" + GeneExpression.EXTENSION)
								+ compMatrix.getValue(rowName, "STAT_" + name + "_FDRBY" + Tiling.EXTENSION)) / 2;
						if (mean < 0)
						...
						// ARRAYS_CORR
						double ratio = OmicsData.MISSING_VALUE;
	...
	private static void summarizeInHTML(String path, String comparison) throws IOException {
		...
		// list =
		// TabDelimitedTableReader.readList(path+comparison+"_Operon_Under_List.txt");
		// html = html.replaceAll("VMinusOperons", list.size()+"");
		// list =
		// TabDelimitedTableReader.readList(path+comparison+"_Operon_Over_List.txt");
		// html = html.replaceAll("VPlusOperons", list.size()+"");
		// list =
		// TabDelimitedTableReader.readList(path+comparison+"_NcRNA_Over_List.txt");
		// html = html.replaceAll("VPlusncRNAs", list.size()+"");
		// list =
		// TabDelimitedTableReader.readList(path+comparison+"_NcRNA_Under_List.txt");
	...
	/**
	 * Run the comparison by calculating statistical parameters given in tests
	 * ArrayList
	 * 
	...
	public static ExpressionMatrix calc(Comparison comp) {
		...
		if (comp.isTiling()) {
			...
			// load data
			TreeMap<String, GeneExpression> bioConds = GeneExpression.getGExpressionData(comp.getLeftBCs(),
					comp.getRightBCs());
							...
							if (test.getType() == TypeStat.FC) {
								...
								compExpression.setValue(testResult, gElement,
										ColorMapper.TypeMapper.CORR + "_" + compTemp.getLeftBCs().get(i) + " vs "
												+ compTemp.getRightBCs().get(i) + "_" + TypeStat.ARRAYSCORR + data);
								// }else if(test.getType()==TypeStat.LNFCWT){
								// double testResult = StatUtils.foldChangeGeneExpression(leftValue,
								// meanWTvalue,true);
								// compExpression.setValue(testResult, gElement,
								// ColorMapper.TypeMapper.LOGFC+"_"+compTemp.getLeftBCs().get(i)+" vs
								// "+compTemp.getRightBCs().get(i)+"_"+TypeStat.LNFCWT+data);
							} else if (test.getType() == TypeStat.LPE) {
								double testResult = LPE.calcP(leftValue, rightValue,
										estimateLPE.get(compTemp.getLeftBCs().get(i)),
								...
								// "+compTemp.getRightBCs().get(i)+"_"+TypeStat.LNFCWT+data);
								// calc LPE
								testResult = LPE.calcP(leftValue, rightValue,
										estimateLPE.get(compTemp.getLeftBCs().get(i)),
										estimateLPE.get(compTemp.getRightBCs().get(i)));
								compExpression.setValue(testResult, gElement, "STAT_" + compTemp.getLeftBCs().get(i)
	...
	private static ExpressionMatrix createCompExpressionMatrix(Genome genome, Comparison comp) {
		...
		for (int i = 0; i < comp.getLeftBCs().size(); i++) {
			...
			for (StatTest test : comp.getTests()) {
				...
				// else if(test.getType()==TypeStat.LNFCWT)
				// headers.add(ColorMapper.TypeMapper.LOGFC+"_"+leftBC+" vs
	 ...
	 * <li>Cell = "+" if a specific gene has been detected over-expressed, and "-"
	 * if under-expressed<br>
	 * <br>
	 * 
