...
public class DNASeqCreation {
	...
	/**
	 * Parse WIG file and calculate Copy Number Variation = raw coverage / mean
	 * coverage<br>
	...
	public static void parseWIGFile() {
		...
		for (BioCondition bioCond : dnaSeqBioConds) {
			for (NGS rnaSeq : bioCond.getNGSSeqs()) {
				...
				ArrayList<String> rows = TabDelimitedTableReader
						.readList(OmicsData.PATH_NGS_RAW + File.separator + fileName, true);
				String finalFileName = OmicsData.PATH_NGS_NORM + File.separator + fileName;
				// try {
				...
				for (String row : rows) {
					if (row.contains("variableStep")) {
						...
						values.add(coverage);
						if (begin % 100000 == 0) {
							System.out.println("Pos: " + begin + " chromo: " + chromoID + " " + coverage);
						}
					}
				}
	...
	/**
	 * For each RNAseq and Sequence element, calculate the mean value of copy number
	 * variation THIS NEED TO BE RUN AFTER CONVERSION of RNASeq files
	 */
	public static void calcGeneCNV() {
		...
		for (String comparison : comparisons) {
			...
			String[][] cnv1 = TabDelimitedTableReader
					.read(OmicsData.PATH_NGS_NORM + bioCond1.getName() + NGS.EXTENSION);
			String[][] cnv2 = TabDelimitedTableReader
					.read(OmicsData.PATH_NGS_NORM + bioCond2.getName() + NGS.EXTENSION);
			...
			TabDelimitedTableReader.save(diff,
					OmicsData.PATH_NGS_NORM + bioCond1.getName() + " vs " + bioCond2.getName() + NGS.EXTENSION);
