    ...
    private static BioCondition createBioCondFromTable(String name, String[][] bioCondTable) {
        ...
        if (!bioCond.isNoData()) {
            for (int i = 1; i < bioCondTable.length; i++) {
                TypeData dataType = TypeData.valueOf(bioCondTable[i][ArrayUtils.findColumn(bioCondTable, "Technology")]);
                ...
                if (!rawDataName.equals("")) {
                    ...
                    } else if (dataType == TypeData.RNASeq 
                            || dataType == TypeData.DNASeq || dataType == TypeData.RiboSeq
                            || dataType == TypeData.TermSeq || dataType == TypeData.TSS) {
