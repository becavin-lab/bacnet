...
public class BioConditionCreation {
	...
	/**
	 * Read a Table summarizing all BioCondition and parse every information
	 * available<br>
	...
	/**
	 * Read the table summarizing all the experiments done, and separate in several
	 * table according to BioCondName = 4th column
	 * 
	...
	/**
	 * From bioCondTable we extract the different Data using the many columns
	...
	private static BioCondition createBioCondFromTable(String name, String[][] bioCondTable) {
		...
		if (!bioCond.isNoData()) {
			for (int i = 1; i < bioCondTable.length; i++) {
				TypeData dataType = TypeData
						.valueOf(bioCondTable[i][ArrayUtils.findColumn(bioCondTable, "Technology")]);
				...
				if (!rawDataName.equals("")) {
					if (dataType == TypeData.ExpressionMatrix) {
						...
						if (rawDataName.contains("_f") && rawDataName.contains(".wig")) {
							...
							boolean found = false;
							/*
							 * If different rawdata are available (replicates) we will not create a new
							 * dataset
							 */
	...
	/**
	 * Create a table will all main information on the different BioCondition
	 * available<br>
	...
	public static void createSummaryTranscriptomesTable() {
		...
		String[] titles = { "Data Name", "Type", "Date", "Growth", "TimePoint", "�C", "Mutant", "Media",
				"MediaGrowthProperties", "Strain used", "Strain array", "Reference", "ArrayExpressId",
		...
		for (BioCondition bioCondition : BioCondition.getAllBioConditions()) {
			if (!bioCondition.getTypeDataContained().contains(TypeData.Proteome)
				...
				String typeDatacontained = bioCondition.getTypeDataContained().toString().replace('[', ' ').replace(']',
						' ');
		...
		for (BioCondition bioCondition : BioCondition.getAllBioConditions()) {
			if (!bioCondition.getTypeDataContained().contains(TypeData.Proteome)
				...
				for (String comparison : bioCondition.getComparisonNames()) {
					...
					row += bioCondition.getMediaGrowthProperties().toString().replace('[', ' ').replace(']', ' ').trim()
							+ "\t";
					String typeDatacontained = bioCondition.getTypeDataContained().toString().replace('[', ' ')
							.replace(']', ' ');
					if (bioCondition.getTypeDataContained().size() == 0)
						typeDatacontained = "GeneExpr";
					if (typeDatacontained.contains("ExpressionMatrix"))
	...
	public static void createSummaryProteomesTable() {
		ArrayList<String> tableResult = new ArrayList<>();
		String[] titles = { "Data Name", "Localization", "Type", "Date", "Nb proteins", "Comment", "Growth",
				"TimePoint", "�C", "Mutant", "Media", "MediaGrowthProperties", "Reference", "Strain used",
				"Strain array", };
		String header = "";
		for (String title : titles)
			header += title + "\t";
