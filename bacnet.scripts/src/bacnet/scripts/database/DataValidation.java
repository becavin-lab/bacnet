		...
		for (String biocondName : this.getTranscriptomes().keySet()) {
			if (!this.getTranscriptomes().get(biocondName)) {
				...
				for (NGS transData : bioCond.getNGSSeqs()) {
					String chromosome = Genome.loadGenome(transData.getGenomeName()).getFirstChromosome()
							.getChromosomeID();
					String fileNameInfo = OmicsData.PATH_STREAMING + transData.getName() + "_" + chromosome
							+ NGS.EXTENSION;
					if (!FileUtils.exists(fileNameInfo)) {
						validate = false;
					}
