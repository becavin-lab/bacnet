...
makeCels=function(celFile,grDir,filecats,tempCELFile){

stopifnot(ncol(filecats)==2 & length(filecats[,1])==length(filecats[,2]))

adfData = data.frame(
    filename = filecats[,1],
	nucleicAcid = filecats[,2])
rownames(adfData) = adfData$filename

levels = paste(unlist(levels(as.factor(as.character(filecats[,2])))),collapse=",")
